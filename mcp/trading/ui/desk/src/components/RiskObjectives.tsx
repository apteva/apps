...
export function RiskObjectives({ portfolioId, policy: inputPolicy, state, objectives, universe: inputUniverse, allowedClasses, onRefresh }: {
  ...
  useEffect(() => { if (!dirty.current) setPolicy(inputPolicy); }, [inputPolicy]);
  useEffect(() => { if (dirty.current) return; setUniverse(inputUniverse); setIncludes((inputUniverse?.include_symbols ?? []).join(", ")); setExcludes((inputUniverse?.exclude_symbols ?? []).join(", ")); }, [inputUniverse]);
  const mutate = async (action: () => Promise<unknown>) => {
    if (busy) return; setBusy(true); setError(null);
    try { await action(); dirty.current = false; onRefresh(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); }
  };
  const savePreset = async (risk_level: RiskPolicy["risk_level"]) => { await mutate(() => updateRisk(portfolioId, { risk_level })); };
  const saveCustom = async () => { if (policy) { await mutate(() => updateRisk(portfolioId, { ...policy, risk_level: "custom" })); } };
  const saveUniverse = async () => { if (universe) { await mutate(() => updateUniversePolicy(portfolioId, { ...universe, include_symbols: splitSymbols(includes), exclude_symbols: splitSymbols(excludes) })); } };
  return <div onChangeCapture={() => { dirty.current = true; }} className="grid lg:grid-cols-2 gap-3 p-2">
    {error && <div role="alert" className="lg:col-span-2">{error}</div>}
    <div className="glass rounded-xl p-4">
      <div className="text-xs uppercase tracking-wider t-tertiary mb-3">Enforced risk policy</div>
      <div className="flex gap-2 mb-4">{(["conservative", "balanced", "aggressive"] as const).map((level) => <button disabled={busy} key={level} onClick={() => savePreset(level)} className={`tab capitalize ${policy?.risk_level === level ? "active" : ""}`}>{level}</button>)}</div>
      ...
      <button disabled={busy} className="btn-primary mt-3" onClick={saveCustom}>Save custom limits</button></>}
    ...
    <div className="glass rounded-xl p-4">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3"><input className="field" value={name} onChange={(e) => setName(e.target.value)}/><input className="field mono" type="number" value={target} onChange={(e) => setTarget(Number(e.target.value))}/><select className="field col-span-2" value={metric} onChange={(e) => setMetric(e.target.value)}><option value="period_return_pct">Return from now</option><option value="total_return_pct">Total return</option><option value="day_return_pct">Day return</option><option value="drawdown_pct">Drawdown floor</option></select><button disabled={busy} className="btn-primary col-span-2" onClick={async () => { await mutate(() => createObjective(portfolioId, { name, metric, target_pct: target, direction: metric === "drawdown_pct" ? "at_most" : "at_least" })); }}>Create objective</button></div>
      <div className="space-y-2">{objectives.length === 0 ? <div className="text-xs t-tertiary py-6 text-center">No percentage objectives yet.</div> : objectives.map((o) => <div key={o.id} className="rounded-lg border border-[var(--border)] p-3"><div className="flex justify-between text-xs"><strong>{o.name}</strong><span className={o.achieved ? "t-up" : "t-secondary"}>{o.achieved ? "achieved" : o.period_state}</span></div><div className="text-[11px] t-tertiary mt-1 mono">{o.actual_pct?.toFixed(2) ?? "—"}% / {o.target_pct.toFixed(2)}%</div><div className="h-1 rounded bg-[var(--surface-2)] mt-2 overflow-hidden"><div className="h-full bg-[var(--accent)]" style={{ width: `${Math.max(0, Math.min(100, o.progress_pct ?? 0))}%` }}/></div><button disabled={busy} className="text-[10px] t-tertiary mt-2" onClick={async () => { await mutate(() => archiveObjective(portfolioId, o.id)); }}>Archive</button></div>)}</div>
    </div>
    <div className="glass rounded-xl p-4 lg:col-span-2">
      ...
      {universe && <div className="grid md:grid-cols-3 gap-3">
        ...
        <div className="md:col-span-3 flex items-center gap-3"><span className="text-[11px] t-tertiary">Asset classes: {allowedClasses.join(", ")}</span><button disabled={busy} className="btn-primary ml-auto" onClick={saveUniverse}>Save universe policy</button></div>
      </div>}
    </div>
  </div>;
}
