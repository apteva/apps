...
interface Spec { version: number; ttl_seconds: number; app_install_ids: number[]; connection_ids: number[]; mcp_server_ids: number[]; network_mode: string; integration_mode: string; integration_bindings: any[]; seeds: Seed[]; web_fixtures: WebFixtureSpec[]; protocol_fixtures: ProtocolFixtureSpec[]; snapshot_id?: string }
...
const emptySpec = (): Spec => ({ version: 1, ttl_seconds: 3600, app_install_ids: [], connection_ids: [], mcp_server_ids: [], network_mode: "block", integration_mode: "mock", integration_bindings: [], seeds: [], web_fixtures: [], protocol_fixtures: [] });
...
function Builder({ initial, catalog, onClose, onSaved }: { initial?: Environment; catalog: Catalog; onClose: () => void; onSaved: () => void }) {
  ...
  const [spec, setSpec] = useState<Spec>({ ...emptySpec(), ...(initial?.spec || {}), mcp_server_ids: initial?.spec.mcp_server_ids || [], web_fixtures: initial?.spec.web_fixtures || [], protocol_fixtures: initial?.spec.protocol_fixtures || [] });
