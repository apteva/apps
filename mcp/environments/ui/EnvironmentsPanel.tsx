...
interface Environment { id: string; name: string; description?: string; desired_state: string; spec: Spec; reconcile_status: string; reconcile_failures: number; reconcile_error?: string; reconcile_next_at?: string; degraded_at?: string; active_run?: Run; runtime?: { status: string; apps: any[]; managed_mcps: RuntimeManagedMCP[]; agents: any[]; expires_at: string } }
...
function Detail({ environment, catalog, onEdit, onRefresh }: { environment: Environment; catalog: Catalog; onEdit: () => void; onRefresh: () => void }) {
  ...
  const displayStatus = environment.reconcile_status === "degraded" ? "degraded" : (run?.status || (environment.reconcile_status === "retrying" ? "retrying" : "stopped"));
  ...
  return <section className="mt-4 overflow-hidden rounded-md border border-border">
    ...
    {environment.reconcile_status === "degraded" && <div className="border-b border-red/40 bg-red/10 px-4 py-3 text-xs text-red"><strong>Automatic reconciliation stopped after {environment.reconcile_failures} failures.</strong><span className="ml-1">{environment.reconcile_error} Edit the definition or press Retry to try explicitly.</span></div>}
    {environment.reconcile_status === "retrying" && <div className="border-b border-accent/40 bg-accent/10 px-4 py-3 text-xs text-text"><strong>Provisioning retry {environment.reconcile_failures} is backed off.</strong><span className="ml-1">Next attempt: {environment.reconcile_next_at ? new Date(environment.reconcile_next_at).toLocaleString() : "pending"}. {environment.reconcile_error}</span></div>}
