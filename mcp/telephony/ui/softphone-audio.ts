...
const JITTER_MAX_SAMPLES = SAMPLE_RATE * 0.5;

// Both processors live in the AudioWorklet global scope, so they ship as source
// text and get loaded from a Blob URL. This avoids serving another static asset
// through the app's route table.
...
export class SoftphoneSession {
  ...
  async start(mediaURL: string): Promise<void> {
    ...
    // Constructed inside the user gesture that started the call, so autoplay
  ...
  private openSocket(mediaURL: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      ws.onerror = () => {
        ...
        reject(new Error("softphone websocket error"));
      };
      ws.onclose = () => {
        if (!this.closed) {
          this.closed = true;
