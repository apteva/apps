function _(t){return t}function G(t){let e=new Int16Array(t.length);for(let s=0;s<t.length;s++){let h=Math.max(-1,Math.min(1,t[s]));e[s]=h<0?h*32768:h*32767}return e.buffer}class o{history=new Float32Array(64);phase=0;process(t,e,s){if(e===s)return t;let h=new Float32Array(64+t.length);h.set(this.history),h.set(t,64);let u=e/s,a=Math.min(1,s/e)*0.9,i=[],m=this.phase;for(;m<t.length;m+=u){let c=m+32,p=0,l=0;for(let f=Math.ceil(c-32);f<=Math.floor(c+32);f++){let d=f-c,b=(Math.abs(d)<0.000000001?a:Math.sin(Math.PI*a*d)/(Math.PI*d))*(0.5+0.5*Math.cos(Math.PI*d/32));if(f>=0&&f<h.length)p+=h[f]*b,l+=b}i.push(l?p/l:0)}return this.phase=m-t.length,this.history=h.slice(-64),new Float32Array(i)}}function C(t){let e=0;for(let s=0;s<t.length;s++)e+=t[s]*t[s];return Math.sqrt(e/Math.max(1,t.length))}var y={echoCancellation:!0,noiseSuppression:!1,autoGainControl:!1,inputGainDB:0,highpassFilter:!0};function S(t){let e=t.playbackTargetMs??60,s=t.playbackMinMs??Math.min(60,e),h=t.playbackMaxMs??160;for(let u of[e,s,h])if(!Number.isFinite(u)||u<40||u>160)throw RangeError("Playback buffers must be between 40 and 160 ms");if(s>e||e>h)throw RangeError("Playback buffers require min <= target <= max");return{initialTargetMs:e,minTargetMs:s,maxTargetMs:h,hardMaxMs:320}}function k(t){return{...t.inputDeviceId?{deviceId:{exact:t.inputDeviceId}}:{},echoCancellation:t.echoCancellation,noiseSuppression:t.noiseSuppression,autoGainControl:t.autoGainControl}}function R(t){let e=t.getSettings();return{deviceLabel:t.label||"Default microphone",sampleRate:typeof e.sampleRate==="number"?e.sampleRate:null,channelCount:typeof e.channelCount==="number"?e.channelCount:null,echoCancellation:typeof e.echoCancellation==="boolean"?e.echoCancellation:null,noiseSuppression:typeof e.noiseSuppression==="boolean"?e.noiseSuppression:null,autoGainControl:typeof e.autoGainControl==="boolean"?e.autoGainControl:null}}function M(t){if(!Number.isFinite(t)||t<=0)return null;return 20*Math.log10(t)}function W(t,e,s){let h=new ArrayBuffer(44+e*2),u=new DataView(h),a=(m,c)=>{for(let p=0;p<c.length;p++)u.setUint8(m+p,c.charCodeAt(p))};a(0,"RIFF"),u.setUint32(4,36+e*2,!0),a(8,"WAVE"),a(12,"fmt "),u.setUint32(16,16,!0),u.setUint16(20,1,!0),u.setUint16(22,1,!0),u.setUint32(24,s,!0),u.setUint32(28,s*2,!0),u.setUint16(32,2,!0),u.setUint16(34,16,!0),a(36,"data"),u.setUint32(40,e*2,!0);let i=44;for(let m of t)for(let c=0;c<m.length;c++)u.setInt16(i,m[c],!0),i+=2;return new Blob([h],{type:"audio/wav"})}class q{onLevel;recordAudio;ctx=null;stream=null;capture=null;sink=null;frames=[];samples=0;activeSquares=0;activeSamples=0;peak=0;postPeak=0;limiterReductionDB=0;settings=null;stopped=!1;resampler=new o;ensureOpen(){if(this.stopped)throw this.release(),Error("Microphone test cancelled.")}constructor(t,e=!0){this.onLevel=t;this.recordAudio=e}async start(t,e){try{this.stream=await navigator.mediaDevices.getUserMedia({audio:k(e)}),this.ensureOpen();let s=this.stream.getAudioTracks()[0];if(!s)throw Error("No microphone audio track was returned.");this.settings=R(s);try{this.ctx=new AudioContext({sampleRate:24000,latencyHint:"interactive"})}catch{this.ctx=new AudioContext({latencyHint:"interactive"})}if(this.ctx.state==="suspended")await this.ctx.resume();this.ensureOpen(),await this.ctx.audioWorklet.addModule(t),this.ensureOpen();let h=this.ctx.sampleRate,u=this.ctx.createMediaStreamSource(this.stream);return this.capture=new AudioWorkletNode(this.ctx,"softphone-capture",{processorOptions:{inputGainDB:e.inputGainDB,highpassFilter:e.highpassFilter}}),this.sink=this.ctx.createGain(),this.sink.gain.value=0,this.capture.connect(this.sink).connect(this.ctx.destination),this.capture.port.onmessage=(a)=>{if(this.stopped)return;if(!(a.data instanceof Float32Array)){if(a.data.type==="capture.stats")this.peak=Math.max(this.peak,a.data.pre_peak??0),this.postPeak=Math.max(this.postPeak,a.data.post_peak??0),this.limiterReductionDB=Math.max(this.limiterReductionDB,a.data.limiter_reduction_db??0);return}let i=a.data;if(h!==24000)i=this.resampler.process(i,h,24000);let m=C(i);if(this.onLevel?.(m),this.recordAudio)this.frames.push(new Int16Array(G(i)));if(this.samples+=i.length,m>=0.005){for(let c=0;c<i.length;c++)this.activeSquares+=i[c]*i[c];this.activeSamples+=i.length}},u.connect(this.capture),this.settings}catch(s){throw await this.release(),s}}async stop(){this.stopped=!0;let t=this.settings,e=this.frames,s=this.samples,h=this.activeSamples>0?Math.sqrt(this.activeSquares/this.activeSamples):0,u=this.peak;if(await this.release(),!t||s===0)throw Error("No microphone audio was captured.");return{audio:W(e,s,24000),durationMs:Math.round(s*1000/24000),sampleRate:24000,activeRmsDbfs:M(h),peakDbfs:M(u),postPeakDbfs:M(this.postPeak||u),limiterReductionDb:this.limiterReductionDB,settings:t}}async cancel(){this.stopped=!0,await this.release()}async release(){if(this.capture)this.capture.port.onmessage=null,this.capture.disconnect(),this.capture=null;this.sink?.disconnect(),this.sink=null;for(let t of this.stream?.getTracks()??[])t.stop();if(this.stream=null,this.ctx&&this.ctx.state!=="closed")await this.ctx.close();this.ctx=null,this.onLevel?.(0)}}class P{callbacks;worker=null;ctx=null;stream=null;capture=null;playback=null;sink=null;output=null;muted=!1;closed=!1;micLevel=0;speakerLevel=0;levelTimer=null;pingTimer=null;opened=!1;cancelWorkerStart;microphoneTransportReady=!1;diagnostics={rttMs:null,queueMs:0,targetMs:60,underruns:0,droppedMs:0,maxQueueMs:0,audioContextRate:24000,websocketBufferedBytes:0,microphoneSampleRate:0,microphoneChannelCount:0,echoCancellation:null,noiseSuppression:null,autoGainControl:null,micActiveRmsDbfs:null,micPeakDbfs:null,micPostPeakDbfs:null,micInputGainDb:y.inputGainDB,micLimiterReductionDb:0,captureSequenceGaps:0,playbackSequenceGaps:0,dropEvents:[]};constructor(t={}){this.callbacks=t}get isMuted(){return this.muted}async start(t,e,s,h=y){let u=S(h);this.diagnostics.targetMs=u.initialTargetMs,this.callbacks.onState?.("connecting");try{this.stream=await navigator.mediaDevices.getUserMedia({audio:k(h)}),this.ensureOpen();let a=this.stream.getAudioTracks()[0];if(!a)throw Error("No microphone audio track was returned.");if(a.readyState==="ended")throw Error("Microphone disconnected before audio setup.");a.onmute=()=>this.callbacks.onNotice?.("Microphone input was interrupted by the device or browser."),a.onunmute=()=>this.callbacks.onNotice?.("Microphone input restored."),a.onended=()=>{if(!this.closed)this.fail("Microphone disconnected. Select a microphone and reconnect audio.")};let i=R(a);this.diagnostics={...this.diagnostics,microphoneSampleRate:i.sampleRate??0,microphoneChannelCount:i.channelCount??0,echoCancellation:i.echoCancellation,noiseSuppression:i.noiseSuppression,autoGainControl:i.autoGainControl,micInputGainDb:h.inputGainDB};try{this.ctx=new AudioContext({sampleRate:24000,latencyHint:"interactive"})}catch{this.ctx=new AudioContext({latencyHint:"interactive"})}if(this.ctx.state==="suspended")await this.ctx.resume();if(this.ensureOpen(),h.outputDeviceId&&"setSinkId"in this.ctx)await this.ctx.setSinkId(h.outputDeviceId),this.ensureOpen();await this.ctx.audioWorklet.addModule(e),this.ensureOpen(),this.diagnostics.audioContextRate=this.ctx.sampleRate;let m=this.ctx.createMediaStreamSource(this.stream);this.capture=new AudioWorkletNode(this.ctx,"softphone-capture",{processorOptions:{inputGainDB:h.inputGainDB,highpassFilter:h.highpassFilter}}),this.playback=new AudioWorkletNode(this.ctx,"softphone-playback",{numberOfInputs:0,outputChannelCount:[1],processorOptions:u}),this.capture.port.postMessage({type:"muted",value:this.muted}),this.installWorkletDiagnostics(),this.sink=this.ctx.createGain(),this.sink.gain.value=0,this.capture.connect(this.sink).connect(this.ctx.destination),m.connect(this.capture),this.output=this.ctx.createGain(),this.output.gain.value=h.outputVolume??1,this.playback.connect(this.output).connect(this.ctx.destination),await this.openWorker(t,s),this.ensureOpen(),this.capture.onprocessorerror=this.playback.onprocessorerror=()=>this.fail("Audio processing stopped. Reconnect audio."),this.ctx.onstatechange=()=>{if(this.closed)return;if(this.ctx?.state==="suspended"||this.ctx?.state==="interrupted")this.callbacks.onState?.("reconnecting","Browser paused audio. Reconnect audio to continue.");else if(this.ctx?.state==="running"&&this.microphoneTransportReady)this.callbacks.onState?.("live")},this.levelTimer=setInterval(()=>{this.callbacks.onLevels?.(this.micLevel,this.speakerLevel),this.micLevel*=0.65,this.speakerLevel*=0.65},100)}catch(a){if(this.closed)this.teardown();if(!this.closed)this.fail(a instanceof Error?a.message:"browser audio setup failed");throw a}}ensureOpen(){if(this.closed)throw this.teardown(),Error("Audio session was cancelled.")}async resumeAudio(){if(await this.ctx?.resume(),this.microphoneTransportReady)this.callbacks.onState?.("live")}setOutputVolume(t){if(this.output)this.output.gain.value=Math.max(0,Math.min(1,t))}sendDTMF(t){if(/^[0-9*#]+$/.test(t))this.sendText(JSON.stringify({type:"dtmf",digits:t}))}installWorkletDiagnostics(){if(!this.capture||!this.playback)return;this.capture.port.onmessage=(t)=>{let e=t.data;if(e?.type!=="capture.stats")return;this.micLevel=this.muted?0:e.active_rms??0,this.diagnostics={...this.diagnostics,micActiveRmsDbfs:M(e.active_rms??0),micPeakDbfs:M(e.pre_peak??0),micPostPeakDbfs:M(e.post_peak??0),micInputGainDb:e.input_gain_db??this.diagnostics.micInputGainDb,micLimiterReductionDb:e.limiter_reduction_db??0}},this.playback.port.onmessage=(t)=>{let e=t.data;if(e?.type!=="stats")return;this.speakerLevel=Math.max(this.speakerLevel,e.speaker_level??0),this.diagnostics={...this.diagnostics,queueMs:e.queue_ms??0,targetMs:e.target_ms??60,underruns:e.underruns??0,droppedMs:e.dropped_ms??0,maxQueueMs:e.max_queue_ms??0,playbackSequenceGaps:e.playback_sequence_gaps??0,dropEvents:[...this.diagnostics.dropEvents.filter((s)=>s.direction!=="carrier_to_operator"),...e.drop_events??[]].slice(-100)},this.callbacks.onDiagnostics?.({...this.diagnostics})}}openWorker(t,e){return new Promise((s,h)=>{let u=new Worker(e);this.worker=u;let a=new MessageChannel,i=new MessageChannel;this.capture?.port.postMessage({type:"transport",port:a.port1},[a.port1]),this.playback?.port.postMessage({type:"transport",port:i.port1},[i.port1]);let m=!1,c=setTimeout(()=>{if(m)return;m=!0,h(Error("audio connection timed out"))},1e4),p=(l)=>{if(m)return;if(m=!0,clearTimeout(c),l)h(l);else s()};this.cancelWorkerStart=()=>p(Error("audio session closed")),u.onmessage=(l)=>{if(this.closed){p(Error("audio session closed"));return}let f=l.data;if(f?.type==="socket.open")this.opened=!0,this.startRTTProbe(),p();else if(f?.type==="socket.message")this.handleControl(f.data);else if(f?.type==="socket.close")if(this.stopRTTProbe(),this.microphoneTransportReady=!1,this.opened&&!this.closed)this.callbacks.onState?.("reconnecting","Connection interrupted; retrying…");else p(Error("audio connection closed before it was ready"));else if(f?.type==="socket.failed")p(Error(f.detail||"audio connection lost")),this.fail(f.detail||"audio connection lost");else if(f?.type==="transport.drop"&&f.event)this.diagnostics.dropEvents=[...this.diagnostics.dropEvents,f.event].slice(-100);else if(f?.type==="transport.stats")this.diagnostics.websocketBufferedBytes=f.buffered_bytes??0},u.onerror=()=>{if(p(Error("audio worker failed")),!this.closed)this.fail("Audio worker failed. Reconnect audio.")},u.postMessage({type:"init",mediaURL:t,contextRate:this.ctx?.sampleRate??24000,muted:this.muted,capturePort:a.port2,playbackPort:i.port2},[a.port2,i.port2])})}handleControl(t){if(this.closed)return;try{let e=JSON.parse(t);if(e.type==="dtmf.error"||e.type==="dtmf.sent")this.callbacks.onNotice?.(e.type==="dtmf.sent"?"Keypad tone sent":e.detail||"Keypad tone failed");else if(e.type==="pong"&&typeof e.nonce==="number"&&e.nonce>=0)this.diagnostics.captureSequenceGaps=e.capture_sequence_gaps??this.diagnostics.captureSequenceGaps,this.diagnostics.rttMs=Math.max(0,Math.round(performance.now()-e.nonce)),this.callbacks.onDiagnostics?.({...this.diagnostics});else if(e.type==="call.ended"||e.type==="session.replaced"){this.closed=!0;try{this.callbacks.onState?.("ended",e.type)}finally{this.teardown()}}else if(e.type==="call.error")this.fail(e.detail||"The call could not be connected.");else if(e.type==="peer.disconnected")this.microphoneTransportReady=!1,this.worker?.postMessage({type:"microphone.ready",value:!1}),this.worker?.postMessage({type:"flush"}),this.callbacks.onState?.("reconnecting","Carrier audio interrupted; reconnecting…");else if(e.type==="peer.connected")this.microphoneTransportReady=!0,this.worker?.postMessage({type:"microphone.ready",value:!0}),this.callbacks.onState?.("live",this.opened?void 0:"Audio reconnected")}catch{}}startRTTProbe(){this.stopRTTProbe();let t=()=>{this.sendText(JSON.stringify({type:"ping",nonce:performance.now()})),this.sendDiagnostics()};t(),this.pingTimer=setInterval(t,5000)}sendText(t){this.worker?.postMessage({type:"send.text",data:t})}sendDiagnostics(){let t=this.diagnostics;this.sendText(JSON.stringify({type:"diagnostics",diagnostics:{rtt_ms:t.rttMs,playback_queue_ms:t.queueMs,playback_target_ms:t.targetMs,playback_max_queue_ms:t.maxQueueMs,playback_underruns:t.underruns,playback_dropped_ms:t.droppedMs,websocket_buffered_bytes:t.websocketBufferedBytes,audio_context_rate:t.audioContextRate,microphone_sample_rate:t.microphoneSampleRate,microphone_channel_count:t.microphoneChannelCount,echo_cancellation:t.echoCancellation,noise_suppression:t.noiseSuppression,auto_gain_control:t.autoGainControl,mic_active_rms_dbfs:t.micActiveRmsDbfs,mic_peak_dbfs:t.micPeakDbfs,mic_post_peak_dbfs:t.micPostPeakDbfs,mic_input_gain_db:t.micInputGainDb,mic_limiter_reduction_db:t.micLimiterReductionDb,capture_sequence_gaps:t.captureSequenceGaps,playback_sequence_gaps:t.playbackSequenceGaps,drop_events:t.dropEvents}})),this.callbacks.onDiagnostics?.({...t})}stopRTTProbe(){if(this.pingTimer!==null)clearInterval(this.pingTimer);this.pingTimer=null}setMuted(t){if(this.muted=t,this.worker?.postMessage({type:"muted",value:t}),this.capture?.port.postMessage({type:"muted",value:t}),t)this.sendText(JSON.stringify({type:"interrupt"}))}stop(){let t=!this.closed;this.closed=!0;try{if(t)this.callbacks.onState?.("ended")}finally{this.teardown()}}fail(t){this.closed=!0;try{this.callbacks.onState?.("error",t)}finally{this.teardown()}}teardown(){this.microphoneTransportReady=!1,this.cancelWorkerStart?.(),this.cancelWorkerStart=void 0;try{this.sendDiagnostics()}catch{}if(this.stopRTTProbe(),this.levelTimer!==null)clearInterval(this.levelTimer);this.levelTimer=null;let t=this.worker;if(t?.postMessage({type:"close"}),t)setTimeout(()=>t.terminate(),100);this.worker=null,this.capture?.disconnect(),this.playback?.disconnect(),this.sink?.disconnect(),this.output?.disconnect(),this.output=null,this.stream?.getTracks().forEach((e)=>e.stop()),this.stream=null,this.ctx?.close().catch(()=>{return}),this.ctx=null,this.capture=null,this.playback=null,this.sink=null}}var E=`const FRAME_SAMPLES = Math.round(sampleRate / 50);
...
`;async function w(){return(await navigator.mediaDevices.enumerateDevices()).filter((e)=>e.kind==="audioinput").map((e,s)=>({deviceId:e.deviceId,label:e.label||`Microphone ${s+1}`}))}function g(t){let e=new q(t,!1),s,h=!1,u,a=()=>{return h=!0,u??=(async()=>{try{await e.cancel()}finally{if(s)URL.revokeObjectURL(s);s=void 0}})()};return{async start(i={}){if(h)throw Error("Microphone preview is already used");h=!0,s=URL.createObjectURL(new Blob([E],{type:"text/javascript"}));try{await e.start(s,{...y,...i})}catch(m){throw await a(),m}},stop:a}}var B={async preflight(t){(await navigator.mediaDevices.getUserMedia({audio:k(t)})).getTracks().forEach((s)=>s.stop())},create(t){let e=new P(t),s=[],h=()=>{try{e.stop()}finally{s.splice(0).forEach((u)=>URL.revokeObjectURL(u))}};return{async start(u,a){try{for(let i of[E,x])s.push(URL.createObjectURL(new Blob([i],{type:"text/javascript"})));await e.start(u,s[0],s[1],a)}catch(i){throw h(),i}},stop:h,setMuted:(u)=>e.setMuted(u),sendDTMF:(u)=>e.sendDTMF(u),setOutputVolume:(u)=>e.setOutputVolume(u)}}};var n=(t)=>t instanceof Error?t.message:String(t);class T{client;options;snapshot=Object.freeze({audioState:"idle",busy:!1,muted:!1});listeners=new Set;audio;session;disposed=!1;generation=0;cancellation=new AbortController;hangingUp;intent;leaseTimer;leaseGeneration=0;timer;polling;runtime;audioOptions;interval;constructor(t,e={}){this.client=t;this.options=e;if(this.runtime=e.audioRuntime??B,this.audioOptions={...y,...e.audio},S(this.audioOptions),this.interval=e.pollIntervalMs??2000,!Number.isFinite(this.interval)||this.interval!==0&&this.interval<100)throw Error("Call poll interval must be 0 or at least 100 ms")}getSnapshot=()=>this.snapshot;subscribe=(t)=>{return this.assertOpen(),this.listeners.add(t),()=>{this.listeners.delete(t)}};update(t){this.snapshot=Object.freeze({...this.snapshot,...t});for(let e of this.listeners)try{e(this.snapshot)}catch{}}assertOpen(){if(this.disposed)throw Error("Softphone has been disposed")}assertCurrent(t){if(this.disposed||t!==this.generation||this.cancellation.signal.aborted)throw Error("Softphone operation cancelled")}invalidate(){return this.cancellation.abort(),this.cancellation=new AbortController,++this.generation}current(t){return!this.disposed&&t===this.generation}finish(t){if(this.current(t))this.update({busy:!1})}cancellable(t,e){let s=this.cancellation.signal;return new Promise((h,u)=>{let a=()=>{s.removeEventListener("abort",a),u(Error("Softphone operation cancelled"))};if(s.addEventListener("abort",a,{once:!0}),t.then(h,u).finally(()=>s.removeEventListener("abort",a)),!this.current(e)||s.aborted)a()})}begin(t){if(this.assertOpen(),this.snapshot.busy||this.hangingUp||t&&this.session)throw Error("Softphone already has an active operation or call");let e=this.invalidate();return this.update({busy:!0,detail:void 0}),e}async dial(t){let e=this.begin(!0),s,h=!1;try{this.assertCurrent(e);let u={to:t.to.trim(),from:t.from?.trim(),timeout_sec:t.timeout_sec,recording:t.recording},a=JSON.stringify(u);if(!this.intent||this.intent.value!==a||t.idempotency_key&&t.idempotency_key!==this.intent.request.idempotency_key)this.intent={value:a,request:{...u,idempotency_key:t.idempotency_key||crypto.randomUUID()}};return await this.cancellable(this.runtime.preflight(this.audioOptions),e),this.assertCurrent(e),s=await this.client.place(this.intent.request),this.intent=void 0,this.assertCurrent(e),h=!0,await this.attachAudio(s,e),s.call_id}catch(u){if(s&&(this.current(e)||!h||this.disposed))await this.recoverStartup(s,e,()=>this.client.hangup(s.call_id));if(this.current(e))this.update({detail:n(u)});throw u}finally{this.finish(e)}}async answer(t,e={}){let s=this.begin(!0),h,u=!1;try{this.assertCurrent(s),h=await this.client.answer(t,e),this.assertCurrent(s),u=!0,await this.attachAudio(h,s)}catch(a){if(h&&(this.current(s)||!u||this.disposed))await this.recoverStartup(h,s,()=>this.client.release(h));if(this.current(s))this.update({detail:n(a)});throw a}finally{this.finish(s)}}async recoverStartup(t,e,s){try{if(await s(),this.current(e))this.clearCall()}catch{if(this.current(e))this.session=t,this.update({callId:t.call_id,audioState:"error"}),this.startPolling()}}async join(t){await this.answer(t,{rejoin:!0})}async attach(t){await this.acquireSession(t,!1)}async takeover(t){await this.acquireSession(t,!0)}async acquireSession(t,e){let s=this.begin(!0);try{await this.cancellable(this.runtime.preflight(this.audioOptions),s),this.assertCurrent(s);let h=await(e?this.client.takeover(t):this.client.attach(t));this.assertCurrent(s),await this.attachAudio(h,s)}catch(h){if(this.current(s))this.update({detail:n(h)});throw h}finally{this.finish(s)}}async reconnect(t){if(!this.session)throw Error("No call to reconnect");let e={...this.audioOptions,...t};S(e);let s=this.begin(!1);this.audioOptions=e;try{await this.attachAudio(this.session,s)}catch(h){if(this.current(s))this.update({detail:n(h)});throw h}finally{this.finish(s)}}async hangup(){if(this.assertOpen(),this.hangingUp)return this.hangingUp;let t=this.session;if(!t){this.cancellation.abort();return}let e=this.invalidate();this.stopAudio(),this.update({busy:!0,audioState:"ended"});let s=(async()=>{try{if(await this.client.hangup(t.call_id),this.current(e))this.clearCall()}catch(h){if(this.current(e))this.update({audioState:"error",detail:n(h)}),this.startPolling();throw h}finally{this.finish(e)}})();this.hangingUp=s;try{await s}finally{if(this.hangingUp===s)this.hangingUp=void 0}}configureAudio(t){this.assertOpen();let e={...this.audioOptions,...t};S(e),this.audioOptions=e}setMuted(t){this.assertOpen(),this.audio?.setMuted(t),this.update({muted:t})}setOutputVolume(t){if(this.assertOpen(),!Number.isFinite(t)||t<0||t>1)throw Error("Volume must be between 0 and 1");this.audioOptions.outputVolume=t,this.audio?.setOutputVolume(t)}sendDTMF(t){if(this.assertOpen(),!/^[0-9*#]+$/.test(t))throw Error("Invalid DTMF digits");if(this.snapshot.audioState!=="live")throw Error("Audio is not connected");this.audio?.sendDTMF(t)}observeCall(t){if(this.disposed||t.id!==this.session?.call_id)return;if(F(t.status))this.invalidate(),this.clearCall(t.status),this.update({busy:!1});else this.update({carrierStatus:t.status})}async attachAudio(t,e){this.assertCurrent(e),this.stopAudio(),this.session=t,this.update({callId:t.call_id,carrierStatus:void 0,audioState:"connecting"});let s,h=()=>!this.disposed&&s!==void 0&&this.audio===s,u=(a)=>{if(h())try{a()}catch{}};try{if(this.assertCurrent(e),s=this.runtime.create({onState:(a,i)=>{if(!h())return;if(a==="ended"&&i==="call.ended"){this.observeCall({id:t.call_id,status:"completed"});return}if(this.update({audioState:a,detail:i}),h()&&(a==="error"||a==="ended")){if(this.stopAudio(),a==="error")this.reconcileFailedAudio(t,e)}},onLevels:(a,i)=>u(()=>this.options.onLevels?.(a,i)),onDiagnostics:(a)=>u(()=>this.options.onDiagnostics?.(a)),onNotice:(a)=>u(()=>this.options.onNotice?.(a))}),this.audio=s,this.assertCurrent(e),this.startPolling(),this.startLease(t),s.setMuted(this.snapshot.muted),await this.cancellable(s.start(this.client.mediaURL(t),this.audioOptions),e),this.assertCurrent(e),!h())throw Error(this.snapshot.detail||"Audio connection ended during setup");s.setMuted(this.snapshot.muted)}catch(a){if(h())this.stopAudio();if(this.current(e))this.update({audioState:"error",detail:n(a)});throw a}}async reconcileFailedAudio(t,e){try{let s=await this.client.getCall(t.call_id);if(!this.current(e)||this.session!==t||!s)return;if(s.status==="pending")this.invalidate(),this.clearCall("pending"),this.update({busy:!1,detail:"The call was not connected. Answer again to retry."});else this.observeCall(s)}catch{}}startLease(t){let e=++this.leaseGeneration;if(!t.lease_seconds)return;let s=async()=>{if(this.disposed||this.session!==t||e!==this.leaseGeneration)return;try{if(await this.client.renew(t),e===this.leaseGeneration)this.leaseTimer=setTimeout(s,t.lease_seconds*1000/3)}catch(h){if(e!==this.leaseGeneration)return;this.stopAudio(),this.update({audioState:"error",detail:`Audio authorization ended: ${n(h)}`})}};this.leaseTimer=setTimeout(s,t.lease_seconds*1000/3)}stopAudio(){++this.leaseGeneration,clearTimeout(this.leaseTimer),this.leaseTimer=void 0;let t=this.audio;this.audio=void 0;try{t?.stop()}catch{}if(t)try{this.options.onLevels?.(0,0)}catch{}}stopPolling(){clearTimeout(this.timer),this.timer=void 0,this.polling?.abort(),this.polling=void 0}startPolling(){if(this.stopPolling(),!this.interval)return;let t=new AbortController;this.polling=t;let e=async()=>{let s=this.session?.call_id;if(!s||t.signal.aborted)return;try{let h=await this.client.getCall(s,t.signal);if(!t.signal.aborted&&h)this.observeCall(h)}catch(h){if(!t.signal.aborted)this.update({detail:`Call status unavailable: ${n(h)}`})}finally{if(!t.signal.aborted&&this.session&&!this.disposed)this.timer=setTimeout(e,this.interval)}};this.timer=setTimeout(e,this.interval)}clearCall(t){this.stopAudio(),this.stopPolling(),this.session=void 0,this.update({callId:void 0,carrierStatus:t,audioState:"idle",muted:!1,detail:void 0})}dispose(){if(this.disposed)return;this.disposed=!0,this.invalidate(),this.listeners.clear(),this.clearCall(),this.update({busy:!1})}}function X(t){return t.direction==="inbound"&&t.status==="pending"&&!t.routing_waiting&&(t.peer_kind==="human"||Boolean(t.ring_offers?.some((e)=>e.kind==="browser")))}var F=(t)=>["completed","failed","no-answer","no_answer","busy","canceled","cancelled"].includes(t);function r(t){if(!t||/[\s/\\?#]/.test(t))throw Error("Invalid call ID");return encodeURIComponent(t)}class A{app;options;listMicrophones=w;createMicrophonePreview=g;constructor(t,e={}){this.app=t;this.options=e;if(t.name!=="telephony"||!t.projectId||!t.installId)throw Error("Telephony requires an explicit project and installation")}path(t){if(!this.options.authProvider)return t;return`/user${t}${t.includes("?")?"&":"?"}auth_provider=${encodeURIComponent(this.options.authProvider)}`}async listCalls(t){let e=await this.app.get(this.path("/calls"),{signal:t});if(!Array.isArray(e?.calls)||e.calls.some((s)=>!s||typeof s.id!=="string"||typeof s.status!=="string"))throw Error("Invalid Telephony calls response");return e.calls}async getCall(t,e){let s=await this.app.get(this.path(`/calls?call_id=${r(t)}`),{signal:e});if(!Array.isArray(s?.calls))throw Error("Invalid Telephony call response");return s.calls.find((h)=>h.id===t)}incomingCalls(t){return t.filter(X)}watchCalls(t,e={}){let s=e.intervalMs??2000;if(!Number.isFinite(s)||s<100)throw Error("Call watch interval must be at least 100 ms");let h=new AbortController,u,a=()=>{clearTimeout(u),h.abort(),e.signal?.removeEventListener("abort",a)};if(e.signal?.addEventListener("abort",a,{once:!0}),e.signal?.aborted)a();let i=async()=>{if(h.signal.aborted)return;try{let m=await this.listCalls(h.signal);if(!h.signal.aborted)t(m)}catch(m){if(!h.signal.aborted)try{e.onError?.(m)}catch{a()}}finally{if(!h.signal.aborted)u=setTimeout(i,s)}};return i(),{close:a}}async place(t){if(!/^\+[1-9]\d{7,14}$/.test(t.to)||!t.idempotency_key?.trim())throw Error("Dial requires an E.164 number and an idempotency key");return this.session(await this.app.post(this.path("/softphone/place"),t))}async answer(t,e={}){return this.session(await this.app.post(this.path(`/softphone/answer/${r(t)}`),e),t)}async attach(t){return this.session(await this.app.post(this.path(`/softphone/attach/${r(t)}`),{}),t)}async takeover(t){return this.session(await this.app.post(this.path(`/softphone/takeover/${r(t)}`),{}),t)}async renew(t){await this.app.post(this.path(`/softphone/renew/${r(t.call_id)}`),{session_token:t.session_token})}async release(t){if(!t.session_token)throw Error("Answer session has no release token");await this.app.post(this.path(`/softphone/release/${r(t.call_id)}`),{session_token:t.session_token})}async hangup(t){await this.app.post(this.path(`/calls/${r(t)}/hangup`),{})}createSoftphone(t={}){return new T(this,t)}mediaURL(t){let e=new URL(this.app.mcpURL(),typeof location>"u"?void 0:location.href),s=new URL(t.media_url,e),h=`/api/apps/telephony/_install/${this.app.installId}/softphone/media/${r(t.call_id)}/`;if(s.origin!==e.origin||!["http:","https:"].includes(s.protocol)||s.username||s.password||s.search||s.hash||!s.pathname.startsWith(h)||!/^[A-Za-z0-9_-]+$/.test(s.pathname.slice(h.length)))throw Error("Invalid Telephony media endpoint");return s.protocol=s.protocol==="https:"?"wss:":"ws:",s.href}session(t,e){let s=t;if(!s||typeof s.call_id!=="string"||typeof s.media_url!=="string"||e&&s.call_id!==e||s.session_token!==void 0&&typeof s.session_token!=="string")throw Error("Invalid Telephony session response");if(s.lease_seconds!==void 0&&(!Number.isFinite(s.lease_seconds)||s.lease_seconds<10||s.lease_seconds>3600||!s.session_token))throw Error("Invalid media lease");return r(s.call_id),this.mediaURL(s),s}}var At=_({app:"telephony",create:({app:t})=>new A(t)});function Pt({app:t},e){return new A(t,e)}export{Pt as createClient};
