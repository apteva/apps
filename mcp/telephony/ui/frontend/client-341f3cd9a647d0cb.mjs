...
`;var R=`// Owns the media WebSocket off the React/main thread. AudioWorklet ports feed
...
`;var x={async preflight(t){(await navigator.mediaDevices.getUserMedia({audio:k(t)})).getTracks().forEach((s)=>s.stop())},create(t){let e=new _(t),s=[],h=()=>{try{e.stop()}finally{s.splice(0).forEach((i)=>URL.revokeObjectURL(i))}};return{async start(i,a){try{for(let u of[b,R])s.push(URL.createObjectURL(new Blob([u],{type:"text/javascript"})));await e.start(i,s[0],s[1],a)}catch(u){throw h(),u}},stop:h,setMuted:(i)=>e.setMuted(i),sendDTMF:(i)=>e.sendDTMF(i),setOutputVolume:(i)=>e.setOutputVolume(i)}}};var l=(t)=>t instanceof Error?t.message:String(t);class q{client;options;snapshot=Object.freeze({audioState:"idle",busy:!1,muted:!1});listeners=new Set;audio;session;disposed=!1;generation=0;cancellation=new AbortController;hangingUp;intent;leaseTimer;leaseGeneration=0;timer;polling;runtime;audioOptions;interval;constructor(t,e={}){this.client=t;this.options=e;if(this.runtime=e.audioRuntime??x,this.audioOptions={...y,...e.audio},S(this.audioOptions),this.interval=e.pollIntervalMs??2000,!Number.isFinite(this.interval)||this.interval!==0&&this.interval<100)throw Error("Call poll interval must be 0 or at least 100 ms")}getSnapshot=()=>this.snapshot;subscribe=(t)=>{return this.assertOpen(),this.listeners.add(t),()=>{this.listeners.delete(t)}};update(t){this.snapshot=Object.freeze({...this.snapshot,...t});for(let e of this.listeners)try{e(this.snapshot)}catch{}}assertOpen(){if(this.disposed)throw Error("Softphone has been disposed")}assertCurrent(t){if(this.disposed||t!==this.generation||this.cancellation.signal.aborted)throw Error("Softphone operation cancelled")}invalidate(){return this.cancellation.abort(),this.cancellation=new AbortController,++this.generation}current(t){return!this.disposed&&t===this.generation}finish(t){if(this.current(t))this.update({busy:!1})}cancellable(t,e){let s=this.cancellation.signal;return new Promise((h,i)=>{let a=()=>{s.removeEventListener("abort",a),i(Error("Softphone operation cancelled"))};if(s.addEventListener("abort",a,{once:!0}),t.then(h,i).finally(()=>s.removeEventListener("abort",a)),!this.current(e)||s.aborted)a()})}begin(t){if(this.assertOpen(),this.snapshot.busy||this.hangingUp||t&&this.session)throw Error("Softphone already has an active operation or call");let e=this.invalidate();return this.update({busy:!0,detail:void 0}),e}async dial(t){let e=this.begin(!0),s,h=!1;try{this.assertCurrent(e);let i={to:t.to.trim(),from:t.from?.trim(),timeout_sec:t.timeout_sec,recording:t.recording},a=JSON.stringify(i);if(!this.intent||this.intent.value!==a||t.idempotency_key&&t.idempotency_key!==this.intent.request.idempotency_key)this.intent={value:a,request:{...i,idempotency_key:t.idempotency_key||crypto.randomUUID()}};return await this.cancellable(this.runtime.preflight(this.audioOptions),e),this.assertCurrent(e),s=await this.client.place(this.intent.request),this.intent=void 0,this.assertCurrent(e),h=!0,await this.attachAudio(s,e),s.call_id}catch(i){if(s&&(this.current(e)||!h||this.disposed))await this.recoverStartup(s,e,()=>this.client.hangup(s.call_id));if(this.current(e))this.update({detail:l(i)});throw i}finally{this.finish(e)}}async answer(t,e={}){let s=this.begin(!0),h,i=!1;try{this.assertCurrent(s),h=await this.client.answer(t,e),this.assertCurrent(s),i=!0,await this.attachAudio(h,s)}catch(a){if(h&&(this.current(s)||!i||this.disposed))await this.recoverStartup(h,s,()=>this.client.release(h));if(this.current(s))this.update({detail:l(a)});throw a}finally{this.finish(s)}}async recoverStartup(t,e,s){try{if(await s(),this.current(e))this.clearCall()}catch{if(this.current(e))this.session=t,this.update({callId:t.call_id,audioState:"error"}),this.startPolling()}}async join(t){await this.answer(t,{rejoin:!0})}async attach(t){await this.acquireSession(t,!1)}async takeover(t){await this.acquireSession(t,!0)}async acquireSession(t,e){let s=this.begin(!0);try{await this.cancellable(this.runtime.preflight(this.audioOptions),s),this.assertCurrent(s);let h=await(e?this.client.takeover(t):this.client.attach(t));this.assertCurrent(s),await this.attachAudio(h,s)}catch(h){if(this.current(s))this.update({detail:l(h)});throw h}finally{this.finish(s)}}async reconnect(t){if(!this.session)throw Error("No call to reconnect");let e={...this.audioOptions,...t};S(e);let s=this.begin(!1);this.audioOptions=e;try{await this.attachAudio(this.session,s)}catch(h){if(this.current(s))this.update({detail:l(h)});throw h}finally{this.finish(s)}}async hangup(){if(this.assertOpen(),this.hangingUp)return this.hangingUp;let t=this.session;if(!t){this.cancellation.abort();return}let e=this.invalidate();this.stopAudio(),this.update({busy:!0,audioState:"ended"});let s=(async()=>{try{if(await this.client.hangup(t.call_id),this.current(e))this.clearCall()}catch(h){if(this.current(e))this.update({audioState:"error",detail:l(h)}),this.startPolling();throw h}finally{this.finish(e)}})();this.hangingUp=s;try{await s}finally{if(this.hangingUp===s)this.hangingUp=void 0}}configureAudio(t){this.assertOpen(),this.audioOptions={...this.audioOptions,...t}}setMuted(t){this.assertOpen(),this.audio?.setMuted(t),this.update({muted:t})}setOutputVolume(t){if(this.assertOpen(),!Number.isFinite(t)||t<0||t>1)throw Error("Volume must be between 0 and 1");this.audioOptions.outputVolume=t,this.audio?.setOutputVolume(t)}sendDTMF(t){if(this.assertOpen(),!/^[0-9*#]+$/.test(t))throw Error("Invalid DTMF digits");if(this.snapshot.audioState!=="live")throw Error("Audio is not connected");this.audio?.sendDTMF(t)}observeCall(t){if(this.disposed||t.id!==this.session?.call_id)return;if(w(t.status))this.invalidate(),this.clearCall(t.status),this.update({busy:!1});else this.update({carrierStatus:t.status})}async attachAudio(t,e){this.assertCurrent(e),this.stopAudio(),this.session=t,this.update({callId:t.call_id,carrierStatus:void 0,audioState:"connecting"});let s,h=()=>!this.disposed&&s!==void 0&&this.audio===s,i=(a)=>{if(h())try{a()}catch{}};try{if(this.assertCurrent(e),s=this.runtime.create({onState:(a,u)=>{if(!h())return;if(a==="ended"&&u==="call.ended"){this.observeCall({id:t.call_id,status:"completed"});return}if(this.update({audioState:a,detail:u}),h()&&(a==="error"||a==="ended")){if(this.stopAudio(),a==="error")this.reconcileFailedAudio(t,e)}},onLevels:(a,u)=>i(()=>this.options.onLevels?.(a,u)),onDiagnostics:(a)=>i(()=>this.options.onDiagnostics?.(a)),onNotice:(a)=>i(()=>this.options.onNotice?.(a))}),this.audio=s,this.assertCurrent(e),this.startPolling(),this.startLease(t),s.setMuted(this.snapshot.muted),await this.cancellable(s.start(this.client.mediaURL(t),this.audioOptions),e),this.assertCurrent(e),!h())throw Error(this.snapshot.detail||"Audio connection ended during setup");s.setMuted(this.snapshot.muted)}catch(a){if(h())this.stopAudio();if(this.current(e))this.update({audioState:"error",detail:l(a)});throw a}}async reconcileFailedAudio(t,e){try{let s=await this.client.getCall(t.call_id);if(!this.current(e)||this.session!==t||!s)return;if(s.status==="pending")this.invalidate(),this.clearCall("pending"),this.update({busy:!1,detail:"The call was not connected. Answer again to retry."});else this.observeCall(s)}catch{}}startLease(t){let e=++this.leaseGeneration;if(!t.lease_seconds)return;let s=async()=>{if(this.disposed||this.session!==t||e!==this.leaseGeneration)return;try{if(await this.client.renew(t),e===this.leaseGeneration)this.leaseTimer=setTimeout(s,t.lease_seconds*1000/3)}catch(h){if(e!==this.leaseGeneration)return;this.stopAudio(),this.update({audioState:"error",detail:`Audio authorization ended: ${l(h)}`})}};this.leaseTimer=setTimeout(s,t.lease_seconds*1000/3)}stopAudio(){++this.leaseGeneration,clearTimeout(this.leaseTimer),this.leaseTimer=void 0;let t=this.audio;this.audio=void 0;try{t?.stop()}catch{}if(t)try{this.options.onLevels?.(0,0)}catch{}}stopPolling(){clearTimeout(this.timer),this.timer=void 0,this.polling?.abort(),this.polling=void 0}startPolling(){if(this.stopPolling(),!this.interval)return;let t=new AbortController;this.polling=t;let e=async()=>{let s=this.session?.call_id;if(!s||t.signal.aborted)return;try{let h=await this.client.getCall(s,t.signal);if(!t.signal.aborted&&h)this.observeCall(h)}catch(h){if(!t.signal.aborted)this.update({detail:`Call status unavailable: ${l(h)}`})}finally{if(!t.signal.aborted&&this.session&&!this.disposed)this.timer=setTimeout(e,this.interval)}};this.timer=setTimeout(e,this.interval)}clearCall(t){this.stopAudio(),this.stopPolling(),this.session=void 0,this.update({callId:void 0,carrierStatus:t,audioState:"idle",muted:!1,detail:void 0})}dispose(){if(this.disposed)return;this.disposed=!0,this.invalidate(),this.listeners.clear(),this.clearCall(),this.update({busy:!1})}}function W(t){return t.direction==="inbound"&&t.status==="pending"&&!t.routing_waiting&&(t.peer_kind==="human"||Boolean(t.ring_offers?.some((e)=>e.kind==="browser")))}var w=(t)=>["completed","failed","no-answer","no_answer","busy","canceled","cancelled"].includes(t);function r(t){if(!t||/[\s/\\?#]/.test(t))throw Error("Invalid call ID");return encodeURIComponent(t)}class A{app;options;constructor(t,e={}){this.app=t;this.options=e;if(t.name!=="telephony"||!t.projectId||!t.installId)throw Error("Telephony requires an explicit project and installation")}path(t){if(!this.options.authProvider)return t;return`/user${t}${t.includes("?")?"&":"?"}auth_provider=${encodeURIComponent(this.options.authProvider)}`}async listCalls(t){let e=await this.app.get(this.path("/calls"),{signal:t});if(!Array.isArray(e?.calls)||e.calls.some((s)=>!s||typeof s.id!=="string"||typeof s.status!=="string"))throw Error("Invalid Telephony calls response");return e.calls}async getCall(t,e){let s=await this.app.get(this.path(`/calls?call_id=${r(t)}`),{signal:e});if(!Array.isArray(s?.calls))throw Error("Invalid Telephony call response");return s.calls.find((h)=>h.id===t)}incomingCalls(t){return t.filter(W)}watchCalls(t,e={}){let s=e.intervalMs??2000;if(!Number.isFinite(s)||s<100)throw Error("Call watch interval must be at least 100 ms");let h=new AbortController,i,a=()=>{clearTimeout(i),h.abort(),e.signal?.removeEventListener("abort",a)};if(e.signal?.addEventListener("abort",a,{once:!0}),e.signal?.aborted)a();let u=async()=>{if(h.signal.aborted)return;try{let m=await this.listCalls(h.signal);if(!h.signal.aborted)t(m)}catch(m){if(!h.signal.aborted)try{e.onError?.(m)}catch{a()}}finally{if(!h.signal.aborted)i=setTimeout(u,s)}};return u(),{close:a}}async place(t){if(!/^\+[1-9]\d{7,14}$/.test(t.to)||!t.idempotency_key?.trim())throw Error("Dial requires an E.164 number and an idempotency key");return this.session(await this.app.post(this.path("/softphone/place"),t))}async answer(t,e={}){return this.session(await this.app.post(this.path(`/softphone/answer/${r(t)}`),e),t)}async attach(t){return this.session(await this.app.post(this.path(`/softphone/attach/${r(t)}`),{}),t)}async takeover(t){return this.session(await this.app.post(this.path(`/softphone/takeover/${r(t)}`),{}),t)}async renew(t){await this.app.post(this.path(`/softphone/renew/${r(t.call_id)}`),{session_token:t.session_token})}async release(t){if(!t.session_token)throw Error("Answer session has no release token");await this.app.post(this.path(`/softphone/release/${r(t.call_id)}`),{session_token:t.session_token})}async hangup(t){await this.app.post(this.path(`/calls/${r(t)}/hangup`),{})}createSoftphone(t={}){return new q(this,t)}mediaURL(t){let e=new URL(this.app.mcpURL(),typeof location>"u"?void 0:location.href),s=new URL(t.media_url,e),h=`/api/apps/telephony/_install/${this.app.installId}/softphone/media/${r(t.call_id)}/`;if(s.origin!==e.origin||!["http:","https:"].includes(s.protocol)||s.username||s.password||s.search||s.hash||!s.pathname.startsWith(h)||!/^[A-Za-z0-9_-]+$/.test(s.pathname.slice(h.length)))throw Error("Invalid Telephony media endpoint");return s.protocol=s.protocol==="https:"?"wss:":"ws:",s.href}session(t,e){let s=t;if(!s||typeof s.call_id!=="string"||typeof s.media_url!=="string"||e&&s.call_id!==e||s.session_token!==void 0&&typeof s.session_token!=="string")throw Error("Invalid Telephony session response");if(s.lease_seconds!==void 0&&(!Number.isFinite(s.lease_seconds)||s.lease_seconds<10||s.lease_seconds>3600||!s.session_token))throw Error("Invalid media lease");return r(s.call_id),this.mediaURL(s),s}}var yt=o({app:"telephony",create:({app:t})=>new A(t)});function At({app:t},e){return new A(t,e)}export{At as createClient};
