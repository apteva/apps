function T(t){return t}async function B(t,e){try{await t.audioWorklet.addModule(e)}catch(h){throw Error("Telephony audio processor could not load. Check the site's Content Security Policy and reload after any Telephony update.",{cause:h})}}function X(t){let e=new Int16Array(t.length);for(let h=0;h<t.length;h++){let s=Math.max(-1,Math.min(1,t[h]));e[h]=s<0?s*32768:s*32767}return e.buffer}class F{history=new Float32Array(64);phase=0;process(t,e,h){if(e===h)return t;let s=new Float32Array(64+t.length);s.set(this.history),s.set(t,64);let f=e/h,a=Math.min(1,h/e)*0.9,m=[],c=this.phase;for(;c<t.length;c+=f){let u=c+32,l=0,n=0;for(let i=Math.ceil(u-32);i<=Math.floor(u+32);i++){let M=i-u,g=(Math.abs(M)<0.000000001?a:Math.sin(Math.PI*a*M)/(Math.PI*M))*(0.5+0.5*Math.cos(Math.PI*M/32));if(i>=0&&i<s.length)l+=s[i]*g,n+=g}m.push(n?l/n:0)}return this.phase=c-t.length,this.history=s.slice(-64),new Float32Array(m)}}function N(t){let e=0;for(let h=0;h<t.length;h++)e+=t[h]*t[h];return Math.sqrt(e/Math.max(1,t.length))}var S={echoCancellation:!0,noiseSuppression:!1,autoGainControl:!1,inputGainDB:0,highpassFilter:!0};function k(t){let e=t.playbackTargetMs??60,h=t.playbackMinMs??Math.min(60,e),s=t.playbackMaxMs??160;for(let f of[e,h,s])if(!Number.isFinite(f)||f<40||f>160)throw RangeError("Playback buffers must be between 40 and 160 ms");if(h>e||e>s)throw RangeError("Playback buffers require min <= target <= max");return{initialTargetMs:e,minTargetMs:h,maxTargetMs:s,hardMaxMs:320}}function P(t){return{...t.inputDeviceId?{deviceId:{exact:t.inputDeviceId}}:{},echoCancellation:t.echoCancellation,noiseSuppression:t.noiseSuppression,autoGainControl:t.autoGainControl}}function G(t){let e=t.getSettings();return{deviceLabel:t.label||"Default microphone",sampleRate:typeof e.sampleRate==="number"?e.sampleRate:null,channelCount:typeof e.channelCount==="number"?e.channelCount:null,echoCancellation:typeof e.echoCancellation==="boolean"?e.echoCancellation:null,noiseSuppression:typeof e.noiseSuppression==="boolean"?e.noiseSuppression:null,autoGainControl:typeof e.autoGainControl==="boolean"?e.autoGainControl:null}}function y(t){if(!Number.isFinite(t)||t<=0)return null;return 20*Math.log10(t)}function O(t,e,h){let s=new ArrayBuffer(44+e*2),f=new DataView(s),a=(c,u)=>{for(let l=0;l<u.length;l++)f.setUint8(c+l,u.charCodeAt(l))};a(0,"RIFF"),f.setUint32(4,36+e*2,!0),a(8,"WAVE"),a(12,"fmt "),f.setUint32(16,16,!0),f.setUint16(20,1,!0),f.setUint16(22,1,!0),f.setUint32(24,h,!0),f.setUint32(28,h*2,!0),f.setUint16(32,2,!0),f.setUint16(34,16,!0),a(36,"data"),f.setUint32(40,e*2,!0);let m=44;for(let c of t)for(let u=0;u<c.length;u++)f.setInt16(m,c[u],!0),m+=2;return new Blob([s],{type:"audio/wav"})}class R{onLevel;recordAudio;ctx=null;stream=null;capture=null;sink=null;frames=[];samples=0;activeSquares=0;activeSamples=0;peak=0;postPeak=0;limiterReductionDB=0;settings=null;stopped=!1;resampler=new F;ensureOpen(){if(this.stopped)throw this.release(),Error("Microphone test cancelled.")}constructor(t,e=!0){this.onLevel=t;this.recordAudio=e}async start(t,e){try{this.stream=await navigator.mediaDevices.getUserMedia({audio:P(e)}),this.ensureOpen();let h=this.stream.getAudioTracks()[0];if(!h)throw Error("No microphone audio track was returned.");this.settings=G(h);try{this.ctx=new AudioContext({sampleRate:24000,latencyHint:"interactive"})}catch{this.ctx=new AudioContext({latencyHint:"interactive"})}if(this.ctx.state==="suspended")await this.ctx.resume();this.ensureOpen(),await B(this.ctx,t),this.ensureOpen();let s=this.ctx.sampleRate,f=this.ctx.createMediaStreamSource(this.stream);return this.capture=new AudioWorkletNode(this.ctx,"softphone-capture",{processorOptions:{inputGainDB:e.inputGainDB,highpassFilter:e.highpassFilter}}),this.sink=this.ctx.createGain(),this.sink.gain.value=0,this.capture.connect(this.sink).connect(this.ctx.destination),this.capture.port.onmessage=(a)=>{if(this.stopped)return;if(!(a.data instanceof Float32Array)){if(a.data.type==="capture.stats")this.peak=Math.max(this.peak,a.data.pre_peak??0),this.postPeak=Math.max(this.postPeak,a.data.post_peak??0),this.limiterReductionDB=Math.max(this.limiterReductionDB,a.data.limiter_reduction_db??0);return}let m=a.data;if(s!==24000)m=this.resampler.process(m,s,24000);let c=N(m);if(this.onLevel?.(c),this.recordAudio)this.frames.push(new Int16Array(X(m)));if(this.samples+=m.length,c>=0.005){for(let u=0;u<m.length;u++)this.activeSquares+=m[u]*m[u];this.activeSamples+=m.length}},f.connect(this.capture),this.settings}catch(h){throw await this.release(),h}}async stop(){this.stopped=!0;let t=this.settings,e=this.frames,h=this.samples,s=this.activeSamples>0?Math.sqrt(this.activeSquares/this.activeSamples):0,f=this.peak;if(await this.release(),!t||h===0)throw Error("No microphone audio was captured.");return{audio:O(e,h,24000),durationMs:Math.round(h*1000/24000),sampleRate:24000,activeRmsDbfs:y(s),peakDbfs:y(f),postPeakDbfs:y(this.postPeak||f),limiterReductionDb:this.limiterReductionDB,settings:t}}async cancel(){this.stopped=!0,await this.release()}async release(){if(this.capture)this.capture.port.onmessage=null,this.capture.disconnect(),this.capture=null;this.sink?.disconnect(),this.sink=null;for(let t of this.stream?.getTracks()??[])t.stop();if(this.stream=null,this.ctx&&this.ctx.state!=="closed")await this.ctx.close();this.ctx=null,this.onLevel?.(0)}}class b{callbacks;worker=null;ctx=null;stream=null;capture=null;playback=null;sink=null;output=null;muted=!1;closed=!1;micLevel=0;speakerLevel=0;levelTimer=null;pingTimer=null;opened=!1;cancelWorkerStart;microphoneTransportReady=!1;diagnostics={rttMs:null,queueMs:0,targetMs:60,underruns:0,droppedMs:0,maxQueueMs:0,audioContextRate:24000,websocketBufferedBytes:0,microphoneSampleRate:0,microphoneChannelCount:0,echoCancellation:null,noiseSuppression:null,autoGainControl:null,micActiveRmsDbfs:null,micPeakDbfs:null,micPostPeakDbfs:null,micInputGainDb:S.inputGainDB,micLimiterReductionDb:0,captureSequenceGaps:0,playbackSequenceGaps:0,dropEvents:[]};constructor(t={}){this.callbacks=t}get isMuted(){return this.muted}async start(t,e,h,s=S){let f=k(s);this.diagnostics.targetMs=f.initialTargetMs,this.callbacks.onState?.("connecting");try{this.stream=await navigator.mediaDevices.getUserMedia({audio:P(s)}),this.ensureOpen();let a=this.stream.getAudioTracks()[0];if(!a)throw Error("No microphone audio track was returned.");if(a.readyState==="ended")throw Error("Microphone disconnected before audio setup.");a.onmute=()=>this.callbacks.onNotice?.("Microphone input was interrupted by the device or browser."),a.onunmute=()=>this.callbacks.onNotice?.("Microphone input restored."),a.onended=()=>{if(!this.closed)this.fail("Microphone disconnected. Select a microphone and reconnect audio.")};let m=G(a);this.diagnostics={...this.diagnostics,microphoneSampleRate:m.sampleRate??0,microphoneChannelCount:m.channelCount??0,echoCancellation:m.echoCancellation,noiseSuppression:m.noiseSuppression,autoGainControl:m.autoGainControl,micInputGainDb:s.inputGainDB};try{this.ctx=new AudioContext({sampleRate:24000,latencyHint:"interactive"})}catch{this.ctx=new AudioContext({latencyHint:"interactive"})}if(this.ctx.state==="suspended")await this.ctx.resume();if(this.ensureOpen(),s.outputDeviceId&&"setSinkId"in this.ctx)await this.ctx.setSinkId(s.outputDeviceId),this.ensureOpen();await B(this.ctx,e),this.ensureOpen(),this.diagnostics.audioContextRate=this.ctx.sampleRate;let c=this.ctx.createMediaStreamSource(this.stream);this.capture=new AudioWorkletNode(this.ctx,"softphone-capture",{processorOptions:{inputGainDB:s.inputGainDB,highpassFilter:s.highpassFilter}}),this.playback=new AudioWorkletNode(this.ctx,"softphone-playback",{numberOfInputs:0,outputChannelCount:[1],processorOptions:f}),this.capture.port.postMessage({type:"muted",value:this.muted}),this.installWorkletDiagnostics(),this.sink=this.ctx.createGain(),this.sink.gain.value=0,this.capture.connect(this.sink).connect(this.ctx.destination),c.connect(this.capture),this.output=this.ctx.createGain(),this.output.gain.value=s.outputVolume??1,this.playback.connect(this.output).connect(this.ctx.destination),await this.openWorker(t,h),this.ensureOpen(),this.capture.onprocessorerror=this.playback.onprocessorerror=()=>this.fail("Audio processing stopped. Reconnect audio."),this.ctx.onstatechange=()=>{if(this.closed)return;if(this.ctx?.state==="suspended"||this.ctx?.state==="interrupted")this.callbacks.onState?.("reconnecting","Browser paused audio. Reconnect audio to continue.");else if(this.ctx?.state==="running"&&this.microphoneTransportReady)this.callbacks.onState?.("live")},this.levelTimer=setInterval(()=>{this.callbacks.onLevels?.(this.micLevel,this.speakerLevel),this.micLevel*=0.65,this.speakerLevel*=0.65},100)}catch(a){if(this.closed)this.teardown();if(!this.closed)this.fail(a instanceof Error?a.message:"browser audio setup failed");throw a}}ensureOpen(){if(this.closed)throw this.teardown(),Error("Audio session was cancelled.")}async resumeAudio(){if(await this.ctx?.resume(),this.microphoneTransportReady)this.callbacks.onState?.("live")}setOutputVolume(t){if(this.output)this.output.gain.value=Math.max(0,Math.min(1,t))}sendDTMF(t){if(/^[0-9*#]+$/.test(t))this.sendText(JSON.stringify({type:"dtmf",digits:t}))}installWorkletDiagnostics(){if(!this.capture||!this.playback)return;this.capture.port.onmessage=(t)=>{let e=t.data;if(e?.type!=="capture.stats")return;this.micLevel=this.muted?0:e.active_rms??0,this.diagnostics={...this.diagnostics,micActiveRmsDbfs:y(e.active_rms??0),micPeakDbfs:y(e.pre_peak??0),micPostPeakDbfs:y(e.post_peak??0),micInputGainDb:e.input_gain_db??this.diagnostics.micInputGainDb,micLimiterReductionDb:e.limiter_reduction_db??0}},this.playback.port.onmessage=(t)=>{let e=t.data;if(e?.type!=="stats")return;this.speakerLevel=Math.max(this.speakerLevel,e.speaker_level??0),this.diagnostics={...this.diagnostics,queueMs:e.queue_ms??0,targetMs:e.target_ms??60,underruns:e.underruns??0,droppedMs:e.dropped_ms??0,maxQueueMs:e.max_queue_ms??0,playbackSequenceGaps:e.playback_sequence_gaps??0,dropEvents:[...this.diagnostics.dropEvents.filter((h)=>h.direction!=="carrier_to_operator"),...e.drop_events??[]].slice(-100)},this.callbacks.onDiagnostics?.({...this.diagnostics})}}openWorker(t,e){return new Promise((h,s)=>{let f=new Worker(e);this.worker=f;let a=new MessageChannel,m=new MessageChannel;this.capture?.port.postMessage({type:"transport",port:a.port1},[a.port1]),this.playback?.port.postMessage({type:"transport",port:m.port1},[m.port1]);let c=!1,u=setTimeout(()=>{if(c)return;c=!0,s(Error("audio connection timed out"))},1e4),l=(n)=>{if(c)return;if(c=!0,clearTimeout(u),n)s(n);else h()};this.cancelWorkerStart=()=>l(Error("audio session closed")),f.onmessage=(n)=>{if(this.closed){l(Error("audio session closed"));return}let i=n.data;if(i?.type==="socket.open")this.opened=!0,this.startRTTProbe(),l();else if(i?.type==="socket.message")this.handleControl(i.data);else if(i?.type==="socket.close")if(this.stopRTTProbe(),this.microphoneTransportReady=!1,this.opened&&!this.closed)this.callbacks.onState?.("reconnecting","Connection interrupted; retrying…");else l(Error("audio connection closed before it was ready"));else if(i?.type==="socket.failed")l(Error(i.detail||"audio connection lost")),this.fail(i.detail||"audio connection lost");else if(i?.type==="transport.drop"&&i.event)this.diagnostics.dropEvents=[...this.diagnostics.dropEvents,i.event].slice(-100);else if(i?.type==="transport.stats")this.diagnostics.websocketBufferedBytes=i.buffered_bytes??0},f.onerror=()=>{if(l(Error("audio worker failed")),!this.closed)this.fail("Audio worker failed. Reconnect audio.")},f.postMessage({type:"init",mediaURL:t,contextRate:this.ctx?.sampleRate??24000,muted:this.muted,capturePort:a.port2,playbackPort:m.port2},[a.port2,m.port2])})}handleControl(t){if(this.closed)return;try{let e=JSON.parse(t);if(e.type==="dtmf.error"||e.type==="dtmf.sent")this.callbacks.onNotice?.(e.type==="dtmf.sent"?"Keypad tone sent":e.detail||"Keypad tone failed");else if(e.type==="pong"&&typeof e.nonce==="number"&&e.nonce>=0)this.diagnostics.captureSequenceGaps=e.capture_sequence_gaps??this.diagnostics.captureSequenceGaps,this.diagnostics.rttMs=Math.max(0,Math.round(performance.now()-e.nonce)),this.callbacks.onDiagnostics?.({...this.diagnostics});else if(e.type==="call.ended"||e.type==="session.replaced"){this.closed=!0;try{this.callbacks.onState?.("ended",e.type)}finally{this.teardown()}}else if(e.type==="call.error")this.fail(e.detail||"The call could not be connected.");else if(e.type==="peer.disconnected")this.microphoneTransportReady=!1,this.worker?.postMessage({type:"microphone.ready",value:!1}),this.worker?.postMessage({type:"flush"}),this.callbacks.onState?.("reconnecting","Carrier audio interrupted; reconnecting…");else if(e.type==="peer.connected")this.microphoneTransportReady=!0,this.worker?.postMessage({type:"microphone.ready",value:!0}),this.callbacks.onState?.("live",this.opened?void 0:"Audio reconnected")}catch{}}startRTTProbe(){this.stopRTTProbe();let t=()=>{this.sendText(JSON.stringify({type:"ping",nonce:performance.now()})),this.sendDiagnostics()};t(),this.pingTimer=setInterval(t,5000)}sendText(t){this.worker?.postMessage({type:"send.text",data:t})}sendDiagnostics(){let t=this.diagnostics;this.sendText(JSON.stringify({type:"diagnostics",diagnostics:{rtt_ms:t.rttMs,playback_queue_ms:t.queueMs,playback_target_ms:t.targetMs,playback_max_queue_ms:t.maxQueueMs,playback_underruns:t.underruns,playback_dropped_ms:t.droppedMs,websocket_buffered_bytes:t.websocketBufferedBytes,audio_context_rate:t.audioContextRate,microphone_sample_rate:t.microphoneSampleRate,microphone_channel_count:t.microphoneChannelCount,echo_cancellation:t.echoCancellation,noise_suppression:t.noiseSuppression,auto_gain_control:t.autoGainControl,mic_active_rms_dbfs:t.micActiveRmsDbfs,mic_peak_dbfs:t.micPeakDbfs,mic_post_peak_dbfs:t.micPostPeakDbfs,mic_input_gain_db:t.micInputGainDb,mic_limiter_reduction_db:t.micLimiterReductionDb,capture_sequence_gaps:t.captureSequenceGaps,playback_sequence_gaps:t.playbackSequenceGaps,drop_events:t.dropEvents}})),this.callbacks.onDiagnostics?.({...t})}stopRTTProbe(){if(this.pingTimer!==null)clearInterval(this.pingTimer);this.pingTimer=null}setMuted(t){if(this.muted=t,this.worker?.postMessage({type:"muted",value:t}),this.capture?.port.postMessage({type:"muted",value:t}),t)this.sendText(JSON.stringify({type:"interrupt"}))}stop(){let t=!this.closed;this.closed=!0;try{if(t)this.callbacks.onState?.("ended")}finally{this.teardown()}}fail(t){this.closed=!0;try{this.callbacks.onState?.("error",t)}finally{this.teardown()}}teardown(){this.microphoneTransportReady=!1,this.cancelWorkerStart?.(),this.cancelWorkerStart=void 0;try{this.sendDiagnostics()}catch{}if(this.stopRTTProbe(),this.levelTimer!==null)clearInterval(this.levelTimer);this.levelTimer=null;let t=this.worker;if(t?.postMessage({type:"close"}),t)setTimeout(()=>t.terminate(),100);this.worker=null,this.capture?.disconnect(),this.playback?.disconnect(),this.sink?.disconnect(),this.output?.disconnect(),this.output=null,this.stream?.getTracks().forEach((e)=>e.stop()),this.stream=null,this.ctx?.close().catch(()=>{return}),this.ctx=null,this.capture=null,this.playback=null,this.sink=null}}var A=`const FRAME_SAMPLES = Math.round(sampleRate / 50);
...
`;var d=`// Owns the media WebSocket off the React/main thread. AudioWorklet ports feed
...
`;function _(t=!1){let e=(t?[A]:[A,d]).map((h)=>URL.createObjectURL(new Blob([h],{type:"text/javascript"})));return{urls:e,dispose(){e.splice(0).forEach((h)=>URL.revokeObjectURL(h))}}}async function x(t,e=!1){if(typeof location>"u")return _(e);let h=new URL(t.mcpURL(),location.href);if(h.origin!==location.origin)return _(e);if(t.name!=="telephony"||!t.projectId||!Number.isSafeInteger(t.installId)||t.installId<=0)throw Error("Telephony audio requires a project and installation");return{urls:await Promise.all((e?[A]:[A,d]).map(async(a,m)=>{let c=new TextEncoder().encode(a),u=Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",c)),(M)=>M.toString(16).padStart(2,"0")).join(""),l=`/ui/frontend/${m===0?"worklet":"worker"}-${u}.js`;if(await t.get(l,{cache:"no-cache",redirect:"error",headers:{Accept:"text/plain"}})!==a)throw Error("Telephony audio asset integrity mismatch. Reload after the app update finishes.");let i=new URL(`/api/apps/telephony/_install/${t.installId}${l}`,h);return i.searchParams.set("project_id",t.projectId),i.searchParams.set("install_id",String(t.installId)),i.href})),dispose(){}}}async function C(){return(await navigator.mediaDevices.enumerateDevices()).filter((e)=>e.kind==="audioinput").map((e,h)=>({deviceId:e.deviceId,label:e.label||`Microphone ${h+1}`}))}function W(t,e){let h=new R(t,!1),s,f=!1,a,m=()=>{return f=!0,a??=(async()=>{try{await h.cancel()}finally{s?.dispose(),s=void 0}})()};return{async start(c={}){if(f)throw Error("Microphone preview is already used");f=!0;try{if(s=e?await x(e,!0):_(!0),a)throw s.dispose(),s=void 0,Error("Microphone preview was cancelled");await h.start(s.urls[0],{...S,...c})}catch(u){throw await m(),u}},stop:m}}function Y(t){return{async preflight(e){(await navigator.mediaDevices.getUserMedia({audio:P(e)})).getTracks().forEach((s)=>s.stop())},create(e){let h=new b(e),s,f=!1,a=()=>{f=!0;try{h.stop()}finally{s?.dispose(),s=void 0}};return{async start(m,c){try{if(s=t?await x(t):_(),f)throw Error("Audio session was cancelled");await h.start(m,s.urls[0],s.urls[1],c)}catch(u){throw a(),u}},stop:a,setMuted:(m)=>h.setMuted(m),sendDTMF:(m)=>h.sendDTMF(m),setOutputVolume:(m)=>h.setOutputVolume(m)}}}}var r=(t)=>t instanceof Error?t.message:String(t);class w{client;options;snapshot=Object.freeze({audioState:"idle",busy:!1,muted:!1});listeners=new Set;audio;session;disposed=!1;generation=0;cancellation=new AbortController;hangingUp;intent;leaseTimer;leaseGeneration=0;timer;polling;runtime;audioOptions;interval;constructor(t,e={}){this.client=t;this.options=e;if(this.runtime=e.audioRuntime??Y(t.app),this.audioOptions={...S,...e.audio},k(this.audioOptions),this.interval=e.pollIntervalMs??2000,!Number.isFinite(this.interval)||this.interval!==0&&this.interval<100)throw Error("Call poll interval must be 0 or at least 100 ms")}getSnapshot=()=>this.snapshot;subscribe=(t)=>{return this.assertOpen(),this.listeners.add(t),()=>{this.listeners.delete(t)}};update(t){this.snapshot=Object.freeze({...this.snapshot,...t});for(let e of this.listeners)try{e(this.snapshot)}catch{}}assertOpen(){if(this.disposed)throw Error("Softphone has been disposed")}assertCurrent(t){if(this.disposed||t!==this.generation||this.cancellation.signal.aborted)throw Error("Softphone operation cancelled")}invalidate(){return this.cancellation.abort(),this.cancellation=new AbortController,++this.generation}current(t){return!this.disposed&&t===this.generation}finish(t){if(this.current(t))this.update({busy:!1})}cancellable(t,e){let h=this.cancellation.signal;return new Promise((s,f)=>{let a=()=>{h.removeEventListener("abort",a),f(Error("Softphone operation cancelled"))};if(h.addEventListener("abort",a,{once:!0}),t.then(s,f).finally(()=>h.removeEventListener("abort",a)),!this.current(e)||h.aborted)a()})}begin(t){if(this.assertOpen(),this.snapshot.busy||this.hangingUp||t&&this.session)throw Error("Softphone already has an active operation or call");let e=this.invalidate();return this.update({busy:!0,detail:void 0}),e}async dial(t){let e=this.begin(!0),h,s=!1;try{this.assertCurrent(e);let f={to:t.to.trim(),from:t.from?.trim(),timeout_sec:t.timeout_sec,recording:t.recording},a=JSON.stringify(f);if(!this.intent||this.intent.value!==a||t.idempotency_key&&t.idempotency_key!==this.intent.request.idempotency_key)this.intent={value:a,request:{...f,idempotency_key:t.idempotency_key||crypto.randomUUID()}};return await this.cancellable(this.runtime.preflight(this.audioOptions),e),this.assertCurrent(e),h=await this.client.place(this.intent.request),this.intent=void 0,this.assertCurrent(e),s=!0,await this.attachAudio(h,e),h.call_id}catch(f){if(h&&(this.current(e)||!s||this.disposed))await this.recoverStartup(h,e,()=>this.client.hangup(h.call_id));if(this.current(e))this.update({detail:r(f)});throw f}finally{this.finish(e)}}async answer(t,e={}){let h=this.begin(!0),s,f=!1;try{this.assertCurrent(h),s=await this.client.answer(t,e),this.assertCurrent(h),f=!0,await this.attachAudio(s,h)}catch(a){if(s&&(this.current(h)||!f||this.disposed))await this.recoverStartup(s,h,()=>this.client.release(s));if(this.current(h))this.update({detail:r(a)});throw a}finally{this.finish(h)}}async recoverStartup(t,e,h){try{if(await h(),this.current(e))this.clearCall()}catch{if(this.current(e))this.session=t,this.update({callId:t.call_id,audioState:"error"}),this.startPolling()}}async join(t){await this.answer(t,{rejoin:!0})}async attach(t){await this.acquireSession(t,!1)}async takeover(t){await this.acquireSession(t,!0)}async acquireSession(t,e){let h=this.begin(!0);try{await this.cancellable(this.runtime.preflight(this.audioOptions),h),this.assertCurrent(h);let s=await(e?this.client.takeover(t):this.client.attach(t));this.assertCurrent(h),await this.attachAudio(s,h)}catch(s){if(this.current(h))this.update({detail:r(s)});throw s}finally{this.finish(h)}}async reconnect(t){if(!this.session)throw Error("No call to reconnect");let e={...this.audioOptions,...t};k(e);let h=this.begin(!1);this.audioOptions=e;try{await this.attachAudio(this.session,h)}catch(s){if(this.current(h))this.update({detail:r(s)});throw s}finally{this.finish(h)}}async hangup(){if(this.assertOpen(),this.hangingUp)return this.hangingUp;let t=this.session;if(!t){this.cancellation.abort();return}let e=this.invalidate();this.stopAudio(),this.update({busy:!0,audioState:"ended"});let h=(async()=>{try{if(await this.client.hangup(t.call_id),this.current(e))this.clearCall()}catch(s){if(this.current(e))this.update({audioState:"error",detail:r(s)}),this.startPolling();throw s}finally{this.finish(e)}})();this.hangingUp=h;try{await h}finally{if(this.hangingUp===h)this.hangingUp=void 0}}configureAudio(t){this.assertOpen();let e={...this.audioOptions,...t};k(e),this.audioOptions=e}setMuted(t){this.assertOpen(),this.audio?.setMuted(t),this.update({muted:t})}setOutputVolume(t){if(this.assertOpen(),!Number.isFinite(t)||t<0||t>1)throw Error("Volume must be between 0 and 1");this.audioOptions.outputVolume=t,this.audio?.setOutputVolume(t)}sendDTMF(t){if(this.assertOpen(),!/^[0-9*#]+$/.test(t))throw Error("Invalid DTMF digits");if(this.snapshot.audioState!=="live")throw Error("Audio is not connected");this.audio?.sendDTMF(t)}observeCall(t){if(this.disposed||t.id!==this.session?.call_id)return;if(H(t.status))this.invalidate(),this.clearCall(t.status),this.update({busy:!1});else this.update({carrierStatus:t.status})}async attachAudio(t,e){this.assertCurrent(e),this.stopAudio(),this.session=t,this.update({callId:t.call_id,carrierStatus:void 0,audioState:"connecting"});let h,s=()=>!this.disposed&&h!==void 0&&this.audio===h,f=(a)=>{if(s())try{a()}catch{}};try{if(this.assertCurrent(e),h=this.runtime.create({onState:(a,m)=>{if(!s())return;if(a==="ended"&&m==="call.ended"){this.observeCall({id:t.call_id,status:"completed"});return}if(this.update({audioState:a,detail:m}),s()&&(a==="error"||a==="ended")){if(this.stopAudio(),a==="error")this.reconcileFailedAudio(t,e)}},onLevels:(a,m)=>f(()=>this.options.onLevels?.(a,m)),onDiagnostics:(a)=>f(()=>this.options.onDiagnostics?.(a)),onNotice:(a)=>f(()=>this.options.onNotice?.(a))}),this.audio=h,this.assertCurrent(e),this.startPolling(),this.startLease(t),h.setMuted(this.snapshot.muted),await this.cancellable(h.start(this.client.mediaURL(t),this.audioOptions),e),this.assertCurrent(e),!s())throw Error(this.snapshot.detail||"Audio connection ended during setup");h.setMuted(this.snapshot.muted)}catch(a){if(s())this.stopAudio();if(this.current(e))this.update({audioState:"error",detail:r(a)});throw a}}async reconcileFailedAudio(t,e){try{let h=await this.client.getCall(t.call_id);if(!this.current(e)||this.session!==t||!h)return;if(h.status==="pending")this.invalidate(),this.clearCall("pending"),this.update({busy:!1,detail:"The call was not connected. Answer again to retry."});else this.observeCall(h)}catch{}}startLease(t){let e=++this.leaseGeneration;if(!t.lease_seconds)return;let h=async()=>{if(this.disposed||this.session!==t||e!==this.leaseGeneration)return;try{if(await this.client.renew(t),e===this.leaseGeneration)this.leaseTimer=setTimeout(h,t.lease_seconds*1000/3)}catch(s){if(e!==this.leaseGeneration)return;this.stopAudio(),this.update({audioState:"error",detail:`Audio authorization ended: ${r(s)}`})}};this.leaseTimer=setTimeout(h,t.lease_seconds*1000/3)}stopAudio(){++this.leaseGeneration,clearTimeout(this.leaseTimer),this.leaseTimer=void 0;let t=this.audio;this.audio=void 0;try{t?.stop()}catch{}if(t)try{this.options.onLevels?.(0,0)}catch{}}stopPolling(){clearTimeout(this.timer),this.timer=void 0,this.polling?.abort(),this.polling=void 0}startPolling(){if(this.stopPolling(),!this.interval)return;let t=new AbortController;this.polling=t;let e=async()=>{let h=this.session?.call_id;if(!h||t.signal.aborted)return;try{let s=await this.client.getCall(h,t.signal);if(!t.signal.aborted&&s)this.observeCall(s)}catch(s){if(!t.signal.aborted)this.update({detail:`Call status unavailable: ${r(s)}`})}finally{if(!t.signal.aborted&&this.session&&!this.disposed)this.timer=setTimeout(e,this.interval)}};this.timer=setTimeout(e,this.interval)}clearCall(t){this.stopAudio(),this.stopPolling(),this.session=void 0,this.update({callId:void 0,carrierStatus:t,audioState:"idle",muted:!1,detail:void 0})}dispose(){if(this.disposed)return;this.disposed=!0,this.invalidate(),this.listeners.clear(),this.clearCall(),this.update({busy:!1})}}function Q(t){return t.direction==="inbound"&&t.status==="pending"&&!t.routing_waiting&&(t.peer_kind==="human"||Boolean(t.ring_offers?.some((e)=>e.kind==="browser")))}var H=(t)=>["completed","failed","no-answer","no_answer","busy","canceled","cancelled"].includes(t);function p(t){if(!t||/[\s/\\?#]/.test(t))throw Error("Invalid call ID");return encodeURIComponent(t)}class E{app;options;listMicrophones=C;createMicrophonePreview=(t)=>W(t,this.app);constructor(t,e={}){this.app=t;this.options=e;if(t.name!=="telephony"||!t.projectId||!t.installId)throw Error("Telephony requires an explicit project and installation")}path(t){if(!this.options.authProvider)return t;return`/user${t}${t.includes("?")?"&":"?"}auth_provider=${encodeURIComponent(this.options.authProvider)}`}async listCalls(t){let e=await this.app.get(this.path("/calls"),{signal:t});if(!Array.isArray(e?.calls)||e.calls.some((h)=>!h||typeof h.id!=="string"||typeof h.status!=="string"))throw Error("Invalid Telephony calls response");return e.calls}async getCall(t,e){let h=await this.app.get(this.path(`/calls?call_id=${p(t)}`),{signal:e});if(!Array.isArray(h?.calls))throw Error("Invalid Telephony call response");return h.calls.find((s)=>s.id===t)}incomingCalls(t){return t.filter(Q)}watchCalls(t,e={}){let h=e.intervalMs??2000;if(!Number.isFinite(h)||h<100)throw Error("Call watch interval must be at least 100 ms");let s=new AbortController,f,a,m=!1,c=!1,u=()=>{clearTimeout(f),s.abort(),a?.close(),e.signal?.removeEventListener("abort",u)},l=(i)=>{try{e.onError?.(i)}catch{u()}};if(e.signal?.addEventListener("abort",u,{once:!0}),e.signal?.aborted)u();let n=async(i)=>{if(s.signal.aborted)return;if(m){c=!0;return}clearTimeout(f),m=!0;let M=performance.now();try{let q=await this.listCalls(s.signal);if(!s.signal.aborted){t(q);try{e.onTiming?.({trigger:i,fetchMs:performance.now()-M})}catch{}}}catch(q){if(!s.signal.aborted)l(q)}finally{if(m=!1,!s.signal.aborted)if(c)c=!1,n("push");else f=setTimeout(()=>void n("poll"),h)}};if(n("poll"),!s.signal.aborted&&e.push!==!1&&typeof this.app.subscribe==="function")try{a=this.app.subscribe(this.path("/calls/events"),(i)=>{if(i.type==="calls.changed")n("push");else if(i.type==="access.revoked")a?.close(),n("push")},{transport:"fetch",signal:s.signal,reconnectDelayMs:250,onError:(i)=>{let M=i?.status;if(M===404||M===405||M===501)a?.close()}})}catch{}return{close:u}}async place(t){if(!/^\+[1-9]\d{7,14}$/.test(t.to)||!t.idempotency_key?.trim())throw Error("Dial requires an E.164 number and an idempotency key");return this.session(await this.app.post(this.path("/softphone/place"),t))}async answer(t,e={}){return this.session(await this.app.post(this.path(`/softphone/answer/${p(t)}`),e),t)}async attach(t){return this.session(await this.app.post(this.path(`/softphone/attach/${p(t)}`),{}),t)}async takeover(t){return this.session(await this.app.post(this.path(`/softphone/takeover/${p(t)}`),{}),t)}async renew(t){await this.app.post(this.path(`/softphone/renew/${p(t.call_id)}`),{session_token:t.session_token})}async release(t){if(!t.session_token)throw Error("Answer session has no release token");await this.app.post(this.path(`/softphone/release/${p(t.call_id)}`),{session_token:t.session_token})}async hangup(t){await this.app.post(this.path(`/calls/${p(t)}/hangup`),{})}createSoftphone(t={}){return new w(this,t)}mediaURL(t){let e=new URL(this.app.mcpURL(),typeof location>"u"?void 0:location.href),h=new URL(t.media_url,e),s=`/api/apps/telephony/_install/${this.app.installId}/softphone/media/${p(t.call_id)}/`;if(h.origin!==e.origin||!["http:","https:"].includes(h.protocol)||h.username||h.password||h.search||h.hash||!h.pathname.startsWith(s)||!/^[A-Za-z0-9_-]+$/.test(h.pathname.slice(s.length)))throw Error("Invalid Telephony media endpoint");return h.protocol=h.protocol==="https:"?"wss:":"ws:",h.href}session(t,e){let h=t;if(!h||typeof h.call_id!=="string"||typeof h.media_url!=="string"||e&&h.call_id!==e||h.session_token!==void 0&&typeof h.session_token!=="string")throw Error("Invalid Telephony session response");if(h.lease_seconds!==void 0&&(!Number.isFinite(h.lease_seconds)||h.lease_seconds<10||h.lease_seconds>3600||!h.session_token))throw Error("Invalid media lease");return p(h.call_id),this.mediaURL(h),h}}var Rt=T({app:"telephony",create:({app:t})=>new E(t)});function xt({app:t},e){return new E(t,e)}export{xt as createClient};
