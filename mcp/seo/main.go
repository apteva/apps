...
func (a *App) toolSERPSearch(ctx *sdk.AppCtx, args map[string]any) (any, error) {
	...
	snapshotID, stored, err := persistSERPSnapshot(db, pid, search_engine, keywordIDPtr,
		keywordText, loc.ID, provider.Slug(), providerResponse)
	if err != nil {
		return nil, err
	}
	return map[string]any{
...
func persistSERPSnapshot(db *sql.DB, pid, searchEngine string, keywordID *int64,
	keywordText string, locationID int64, provider string, response *providerSERPResponse) (int64, []SearchRanking, error) {
