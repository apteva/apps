// Apteva Tasks app — mission board for an Apteva agent.
//
...
provides:
  ...
  mcp_tools:
    - name: tasks_create
      description: Create a new task on this agent's mission board.
    - name: tasks_update
      description: Update a task's status, title, or notes.
    - name: tasks_complete
      description: Mark a task as done.
    - name: tasks_list
      description: List the open or recent tasks for an agent.
runtime:
  kind: source
  source:
...
func (a *App) HTTPRoutes() []sdk.Route {
	return []sdk.Route{
		{Pattern: "/agents/", Handler: a.handleAgentsItem}, // GET (list) + POST (create)
		{Pattern: "/tasks/", Handler: a.handleTasksItem},   // PUT (update) + DELETE
	}
}

// handleAgentsItem dispatches /agents/<id> by method.
...
func (a *App) MCPTools() []sdk.Tool {
	return []sdk.Tool{
		{
			Name:        "tasks_create",
			Description: "Create a task on an agent's mission board. Args: agent_id, title, notes (optional).",
...
func dbList(db *sql.DB, agentID int64, status string) ([]Task, error) {
	q := `SELECT id, agent_id, title, COALESCE(notes,''), status, created_at, updated_at FROM tasks WHERE agent_id = ?`
	args := []any{agentID}
	...
	for rows.Next() {
		var t Task
		if err := rows.Scan(&t.ID, &t.AgentID, &t.Title, &t.Notes, &t.Status, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
