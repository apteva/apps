...
const targetLabel = (target?: Target) => {
  if (!target) return "—";
  const name = target.agent_name || target.draft?.name || "";
  return [name, target.model || target.provider].filter(Boolean).join(" · ") || "—";
};
...
function RunForm({ pack, catalog, busy, onSave, onClose }: {
  ...
  const newDraftTarget = (): Target => ({
    draft: { name: "", directive: "", mode: "autonomous", config: "{}" }, provider: "", model: "",
  });
  const newAgentTarget = (): Target => ({ agent_id: 0, provider: "", model: "" });
  const [targets, setTargets] = useState<Target[]>([newDraftTarget()]);
  const [trials, setTrials] = useState(3);
  const [name, setName] = useState("");
  const setTarget = (i: number, patch: Partial<Target>) =>
    setTargets((cur) => cur.map((t, j) => (j === i ? { ...t, ...patch } : t)));
  const setDraft = (i: number, patch: Partial<AgentDraft>) =>
    setTargets((cur) => cur.map((t, j) => j === i ? { ...t, draft: { ...(t.draft || newDraftTarget().draft!), ...patch } } : t));
  const validConfig = (value?: string) => {
    try { return !!JSON.parse(value || "{}"); } catch { return false; }
  };
  const validTarget = (target: Target) => target.draft
    ? !!target.draft.name.trim() && !!target.draft.directive.trim() && validConfig(target.draft.config)
    : (target.agent_id || 0) > 0;
  const valid = targets.length > 0 && targets.every(validTarget);
  ...
  return (
    <Modal title={`Run ${pack.name} v${pack.version}`} onClose={onClose}>
      ...
      <div className="flex items-center justify-between pt-2">
        <span className={labelCls}>Candidate setups — compare as many hidden drafts or existing agents as you need</span>
        <div className="flex gap-2">
          <button className={btn} onClick={() => setTargets([...targets, newDraftTarget()])}>+ Hidden setup</button>
          <button className={btn} onClick={() => setTargets([...targets, newAgentTarget()])}>+ Existing agent</button>
        </div>
      </div>
      {targets.map((t, i) => (
        <div key={i} className="border border-border rounded p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between gap-3">
            <strong className="text-sm">Candidate {i + 1}</strong>
            <div className="flex gap-2">
              <button className={btn} disabled={!!t.draft} onClick={() => setTargets((cur) => cur.map((value, j) => j === i ? newDraftTarget() : value))}>Hidden setup</button>
              <button className={btn} disabled={!t.draft} onClick={() => setTargets((cur) => cur.map((value, j) => j === i ? newAgentTarget() : value))}>Existing agent</button>
          ...
          {t.draft ? <>
            <div>
              ...
              <input className={field} value={t.draft.name} placeholder="Research agent · strict tool policy"
                onChange={(e) => setDraft(i, { name: e.target.value })} />
            </div>
            <div>
              <label className={labelCls}>Directive</label>
              <textarea className={field} rows={5} value={t.draft.directive}
                placeholder="Give this temporary agent its complete role, goals, rules, and tool-use instructions."
                onChange={(e) => setDraft(i, { directive: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className={labelCls}>Mode</label>
                <select className={field} value={t.draft.mode || "autonomous"}
                  onChange={(e) => setDraft(i, { mode: e.target.value as AgentDraft["mode"] })}>
                  <option value="autonomous">Autonomous</option><option value="cautious">Cautious</option><option value="learn">Learn</option>
                </select>
              </div>
              <div>
                <label className={labelCls}>Config JSON</label>
                <input className={field} value={t.draft.config || "{}"}
                  onChange={(e) => setDraft(i, { config: e.target.value })} />
              </div>
            </div>
          </> : <div>
            <label className={labelCls}>Project agent</label>
          ...
          <div>
            ...
            <select className={field} value={t.model || ""}
              onChange={(e) => {
                ...
                setTarget(i, { model: e.target.value, provider: m ? "" : t.provider });
              }}>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button className={btn} disabled={busy || !valid}
          onClick={() => onSave(targets, trials, name)}>Queue {targets.length} setup{targets.length === 1 ? "" : "s"}</button>
      </div>
    </Modal>
  );
}
