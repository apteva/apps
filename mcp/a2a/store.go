...
type Task struct {
	Artifacts         storedJSON `json:"artifacts,omitempty"`
	ID                int64      `json:"id"`
...
func getTask(db *sql.DB, projectID string, id int64) (*Task, error) {
	row := db.QueryRow(`
		SELECT id, project_id, kind, status, from_agent_id, from_agent_name, from_thread_id,
		       ...
		       remote_context_id, last_synced_at, artifacts_json, created_at, updated_at
		FROM a2a_tasks
	...
	err := row.Scan(&t.ID, &t.ProjectID, &t.Kind, &t.Status, &t.FromAgentID, &t.FromAgentName,
		...
		&t.RemoteContextID, &t.LastSyncedAt, &t.Artifacts, &t.CreatedAt, &t.UpdatedAt)
...
func listOpenOutboundTasks(db *sql.DB, projectID string, limit int) ([]*Task, error) {
	return listTasks(db, projectID, taskFilter{Status: "open", Limit: limit, Direction: "outbound", Poll: true})
}
...
func listTasks(db *sql.DB, projectID string, f taskFilter) ([]*Task, error) {
	...
	if f.Poll {
		where = append(where, "next_poll_at <= ?", "remote_task_id <> ''")
		args = append(args, nowUTC())
		order = "last_synced_at ASC, id ASC"
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.ProjectID, &t.Kind, &t.Status, &t.FromAgentID, &t.FromAgentName,
			&t.FromThreadID, &t.ToAgentID, &t.ToAgentName, &t.ToThreadID, &t.Direction, &t.PeerID,
			&t.RemoteCardID, &t.ProtocolTaskID, &t.ProtocolContextID, &t.RemoteTaskID,
			&t.RemoteContextID, &t.LastSyncedAt, &t.Artifacts, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, &t)
	}
...
// SQLite returns TEXT as strings; expose stored JSON without double encoding.
type storedJSON json.RawMessage

func (j *storedJSON) Scan(src any) error {
	switch v := src.(type) {
	case string:
		*j = storedJSON(v)
	case []byte:
...
func (j storedJSON) MarshalJSON() ([]byte, error) {
	if len(j) == 0 {
		return []byte("[]"), nil
	}
	return []byte(j), nil
