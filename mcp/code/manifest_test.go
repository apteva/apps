...
func TestEmbeddedManifest_Valid(t *testing.T) {
	...
	if len(m.Provides.MCPTools) != 53 {
		t.Errorf("expected 53 MCP tools in manifest, got %d", len(m.Provides.MCPTools))
	}
	...
	for _, dependency := range m.Requires.Apps {
		if dependency.Name == "workspaces" && dependency.Version == ">=0.4.0" && dependency.Optional {
			workspacesOptional = true
		}
	}
	if !workspacesOptional {
		t.Error("Workspaces >=0.4.0 must remain an optional dependency")
	}
}
...
func TestMCPTools_EditingSurfaceComplete(t *testing.T) {
	...
	must := []string{
		...
		"repos_run_command", "repos_workspace_changes", "repos_workspace_apply",
