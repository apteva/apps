...
export default function AgentWorldsPanel({ installId, projectId }: Props) {
  ...
  return <div className="aw">
    ...
    <header className="aw-header">
      <div className="aw-heading"><div className="aw-title-row"><span className="aw-mark" aria-hidden="true">◈</span><h1>Agent Worlds</h1><span className="aw-beta">LIVE</span></div><p>See how agents work across your server and environments.</p></div>
    ...
    <main className="aw-main">
      <section className="aw-toolbar" aria-label="Scene controls">
        ...
        <label className="aw-field aw-agent"><span>Agent</span><select value={selectedAgent} onChange={event => setSelectedAgent(event.target.value)}><option value="">All agents</option>{scene?.agents.map(agent => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label>
        <div className="aw-toolbar-actions"><button className={`aw-button aw-live ${live ? "is-live" : ""}`} onClick={() => { setLive(!live); setReplayIndex(scene?.events.length || 0); }}><span className="aw-dot" />{live ? "Live" : "Paused"}</button><button className="aw-button" onClick={loadScene}>Refresh</button></div>
      </section>
      {error && <div className="aw-error" role="alert">{error}</div>}
      <section className="aw-workspace">
        <div className="aw-workspace-bar"><div><div className="aw-eyebrow">VISUALIZATION</div><h2>{activePreset.name}</h2><p>{activePreset.detail}</p></div><label className="aw-portrait"><input type="checkbox" checked={portrait} onChange={event => setPortrait(event.target.checked)} /> Portrait 9:16</label></div>
        <nav className="aw-presets" aria-label="Visualization preset">{presets.map((item, index) => <button key={item.id} type="button" onClick={() => setPreset(item.id)} aria-current={preset === item.id ? "page" : undefined} className={preset === item.id ? "is-active" : ""}><span className="aw-preset-num">0{index + 1}</span>{item.name}</button>)}</nav>
        <div className="aw-content">
          <section className={`aw-stage-card ${portrait ? "is-portrait" : ""}`} aria-label={activePreset.name}>
            <div className="aw-stage-top"><div className="aw-stage-label"><span className="aw-stage-indicator" />{scene?.source.label || "Connecting"}</div><span>{visibleAgents.length} agents · {runningCount} running</span></div>
            <div className={portrait ? "aw-stage aw-stage-portrait" : "aw-stage"}>
              {scene ? preset === "graph" ? <SystemGraph refNode={stageRef} source={scene.source} agents={visibleAgents} apps={scene.apps} events={visibleEvents} portrait={portrait} onSelect={setSelectedAgent} palette={palette} /> : preset === "village" ? <PixelVillage refNode={stageRef} agents={visibleAgents} events={visibleEvents} portrait={portrait} now={animationNow} seenAt={seenAtRef.current} onSelect={setSelectedAgent} palette={palette} /> : preset === "operations" ? <OperationsStage refNode={stageRef} agents={visibleAgents} events={visibleEvents} portrait={portrait} palette={palette} /> : <ConversationStage refNode={stageRef} agents={visibleAgents} events={visibleEvents} portrait={portrait} palette={palette} /> : <div className="aw-loading">Connecting to the scene…</div>}
            </div>
            <div className="aw-stage-footer"><span>Click an agent to focus the view</span><span>{scene ? `Updated ${clock(scene.at)}` : "Waiting for data"}</span></div>
          </section>
          <aside className="aw-aside">
            <section className="aw-panel"><div className="aw-panel-heading"><h3>Overview</h3><span>THIS SOURCE</span></div><div className="aw-metrics"><Metric label="Agents" value={visibleAgents.length} /><Metric label="Running" value={runningCount} /><Metric label="Threads" value={threadCount} /><Metric label="Events" value={visibleEvents.length} /></div></section>
            <section className="aw-panel aw-activity"><div className="aw-panel-heading"><h3>Activity</h3><span>{live ? "LIVE FEED" : "REPLAY"}</span></div><div className="aw-activity-list">{visibleEvents.slice(-10).reverse().map(event => <div key={event.id} className="aw-event"><span className="aw-event-mark" style={{ background: eventColor(event.kind, palette) }} /><div><div className="aw-event-line"><b>{scene?.agents.find(agent => agent.id === event.agent_id)?.name || event.agent_id}</b> {event.label}</div><div className="aw-event-meta">{clock(event.time)}{event.thread_id ? ` · ${short(event.thread_id, 18)}` : ""}</div></div></div>)}{!visibleEvents.length && <div className="aw-empty"><span className="aw-empty-icon">◇</span><strong>No activity yet</strong><span>Events will appear here as agents work.</span></div>}</div></section>
          </aside>
        </div>
...
function Metric({ label, value }: { label: string; value: number }) { return <div className="aw-metric"><strong>{value}</strong><span>{label}</span></div>; }
...
function SystemGraph({ refNode, source, agents, apps, events, portrait, onSelect, palette }: { refNode: React.RefObject<SVGSVGElement | null>; source: Source; agents: Agent[]; apps: AppNode[]; events: Event[]; portrait: boolean; onSelect: (id: string) => void; palette: Palette }) {
  const width = portrait ? 540 : 960, height = portrait ? 960 : 600;
  const latest = latestByAgent(events);
  const resources = [...new Set([...apps.map(app => app.name), ...events.filter(event => event.target).map(event => event.target!)])].slice(0, 5);
  const shown = agents.slice(0, 12), rows = Math.max(1, Math.ceil(shown.length / 2));
  const cardW = portrait ? 220 : 228, cardH = portrait ? Math.min(82, 440 / rows - 10) : Math.min(78, 365 / rows - 10);
  const top = portrait ? 160 : 125, step = portrait ? Math.min(105, 450 / rows) : Math.min(104, 370 / rows);
  const positions = shown.map((agent, i) => ({ agent, x: portrait ? (i % 2 ? 290 : 30) : (i % 2 ? 682 : 50), y: top + Math.floor(i / 2) * step }));
  const hub = { x: portrait ? 270 : 480, y: portrait ? 690 : 300 };
  return <svg ref={refNode} viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" role="img" aria-label="Agent system graph" style={{ color: palette.text, background: palette.subtle, fontFamily: palette.font }}>
    <defs><pattern id="awGraphGrid" width="32" height="32" patternUnits="userSpaceOnUse"><path d="M 32 0 L 0 0 0 32" fill="none" stroke={palette.border} strokeWidth=".7" /></pattern></defs>
    <rect width={width} height={height} fill={palette.subtle} /><rect width={width} height={height} fill="url(#awGraphGrid)" />
    <SvgText x={32} y={45} size={20} weight={700}>System topology</SvgText><SvgText x={32} y={68} size={11} fill={palette.dim}>LIVE CONNECTIONS · {source.label.toUpperCase()}</SvgText>
    <line x1={32} y1={88} x2={width - 32} y2={88} stroke={palette.border} />
    {positions.map(({agent,x,y}) => <path key={`edge:${agent.id}`} d={`M ${hub.x} ${hub.y} L ${x + cardW / 2} ${y + cardH / 2}`} fill="none" stroke={latest.has(agent.id) ? palette.accent : palette.strong} strokeWidth={latest.has(agent.id) ? 2 : 1} strokeDasharray={latest.has(agent.id) ? undefined : "4 5"} opacity=".7" />)}
    <rect x={hub.x - 115} y={hub.y - 47} width="230" height="94" rx="8" fill={palette.card} stroke={palette.accent} strokeWidth="2" /><rect x={hub.x - 115} y={hub.y - 47} width="4" height="94" fill={palette.accent} />
    <SvgText x={hub.x - 92} y={hub.y - 15} size={10} fill={palette.accent} weight={700}>APTEVA / {source.kind.includes("runtime") ? "ENVIRONMENT" : "SERVER"}</SvgText>
    <SvgText x={hub.x - 92} y={hub.y + 14} size={18} weight={700}>{short(source.label, 19)}</SvgText><SvgText x={hub.x - 92} y={hub.y + 34} size={10} fill={palette.dim}>{agents.length} connected agents</SvgText>
    {positions.map(({ agent,x,y }) => <g key={agent.id} onClick={() => onSelect(agent.id)} style={{ cursor: "pointer" }}><rect x={x} y={y} width={cardW} height={cardH} rx="7" fill={palette.card} stroke={palette.border} /><rect x={x} y={y} width="3" height={cardH} fill={statusColor(agent.status, palette)} /><circle cx={x + 25} cy={y + 27} r="12" fill={palette.subtle} stroke={palette.strong} /><SvgText x={x + 25} y={y + 31} size={12} fill={palette.accent} anchor="middle" weight={700}>{agent.name.slice(0,1).toUpperCase()}</SvgText><SvgText x={x + 46} y={y + 26} size={12} weight={700}>{short(agent.name, portrait ? 19 : 21)}</SvgText><circle cx={x + cardW - 16} cy={y + 22} r="4" fill={statusColor(agent.status, palette)} /><SvgText x={x + 15} y={y + cardH - 16} size={10} fill={palette.dim}>{short(latest.get(agent.id)?.label || agent.status, 31)}</SvgText></g>)}
    {!agents.length && <SvgText x={width / 2} y={height / 2 - 80} size={15} fill={palette.dim} anchor="middle">No agents in this source yet</SvgText>}
    {agents.length > shown.length && <SvgText x={width - 32} y={height - 76} size={11} fill={palette.dim} anchor="end">+{agents.length - shown.length} more agents</SvgText>}
    <line x1={32} y1={height - 87} x2={width - 32} y2={height - 87} stroke={palette.border} /><SvgText x={32} y={height - 60} size={10} fill={palette.dim} weight={700}>RESOURCES</SvgText>
    {resources.length ? resources.map((name,i) => { const x = 32 + i * (portrait ? 98 : 178); return <g key={name}><rect x={x} y={height - 47} width={portrait ? 90 : 166} height="25" rx="4" fill={palette.card} stroke={palette.border} /><SvgText x={x + 9} y={height - 30} size={10} fill={palette.muted}>{short(name, portrait ? 11 : 24)}</SvgText></g>; }) : <SvgText x={32} y={height - 30} size={11} fill={palette.dim}>Resources appear when agents use tools or apps.</SvgText>}
  </svg>;
}
