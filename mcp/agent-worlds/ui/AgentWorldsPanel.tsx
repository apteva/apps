...
type Destination = { id: string; kind: "app" | "integration" | "other"; name: string; status: string; tools?: string[]; tool_count: number; call_count: number; last_call?: string };
...
function featuredDestinations(items: Destination[], limit: number, selectedID: string) {
  const ranked = [...items].sort((a, b) => b.call_count - a.call_count || a.name.localeCompare(b.name));
  const featured: Destination[] = [];
  const add = (item?: Destination) => { if (item && featured.length < limit && !featured.some(pick => pick.id === item.id)) featured.push(item); };
  add(items.find(item => item.id === selectedID));
  for (const item of ranked.filter(item => item.call_count > 0)) if (featured.length < limit - 2) add(item);
  for (const kind of ["app", "integration"] as const) for (const item of ranked.filter(item => item.kind === kind).slice(0, 2)) add(item);
        ...
        <div className={`aw-content ${portrait ? "is-portrait" : ""}`}>
          <section className="aw-scene" aria-label={activePreset.name}>
            ...
            <div className="aw-canvas">
              {scene ? preset === "graph" ? <SystemGraph refNode={stageRef} source={scene.source} agents={visibleAgents} destinations={destinations} selectedDestination={selectedDestination} events={visibleEvents} portrait={portrait} onSelect={setSelectedAgent} onSelectDestination={setSelectedDestination} palette={palette} /> : preset === "village" ? <PixelVillage refNode={stageRef} agents={visibleAgents} destinations={destinations} selectedDestination={selectedDestination} events={visibleEvents} portrait={portrait} now={animationNow} seenAt={seenAtRef.current} onSelect={setSelectedAgent} onSelectDestination={setSelectedDestination} palette={palette} /> : preset === "operations" ? <OperationsStage refNode={stageRef} agents={visibleAgents} events={visibleEvents} portrait={portrait} palette={palette} /> : <ConversationStage refNode={stageRef} agents={visibleAgents} events={visibleEvents} portrait={portrait} palette={palette} /> : <div className="aw-loading">Connecting to the scene…</div>}
            </div>
...
function SystemGraph({ refNode, source, agents, destinations, selectedDestination, events, portrait, onSelect, onSelectDestination, palette }: { refNode: React.RefObject<SVGSVGElement | null>; source: Source; agents: Agent[]; destinations: Destination[]; selectedDestination: string; events: Event[]; portrait: boolean; onSelect: (id: string) => void; onSelectDestination: (id: string) => void; palette: Palette }) {
  const width = portrait ? 540 : 1280, height = portrait ? 960 : 560;
  const shownAgents = agents.slice(0, portrait ? 8 : 9), shownDestinations = featuredDestinations(destinations, portrait ? 8 : 9, selectedDestination);
...
function PixelVillage({ refNode, agents, destinations, selectedDestination, events, portrait, now, seenAt, onSelect, onSelectDestination, palette }: { refNode: React.RefObject<SVGSVGElement | null>; agents: Agent[]; destinations: Destination[]; selectedDestination: string; events: Event[]; portrait: boolean; now: number; seenAt: Map<string, number>; onSelect: (id: string) => void; onSelectDestination: (id: string) => void; palette: Palette }) {
  const width = portrait ? 540 : 1280, height = portrait ? 960 : 560;
  const groundY = portrait ? 625 : 385;
  const visibleDestinations = featuredDestinations(destinations, 4, selectedDestination);
