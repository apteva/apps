	...
	return []sdk.Tool{
		// ── Posts + pages ───────────────────────────────────────
		{
			...
			InputSchema: schemaObject(map[string]any{
				...
				"term_kind": map[string]any{"type": "string", "enum": []string{"category", "tag"}},
		...
		{
			Name:        "posts_update",
			Description: "Partial-patch a post or page. Creates a revision; pass expected_version from posts_get to reject stale edits. Args: id (required), plus any subset of title, excerpt, slug, locale, blocks, author, template, featured_media_id, parent_id, menu_order, seo_*, og_image_media_id.",
			InputSchema: schemaObject(map[string]any{
