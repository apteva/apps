...
function useConversationTransport(conversationID: string, projectId: string) {
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      const page = await conversationsClient.history(conversationID, { before: beforeRef.current, limit: 100 });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        if (!initialized) {
          const page = await conversationsClient.history(conversationID, { limit: 100 });
          if (cancelled) return;
        ...
        for (let n=0;n<10 && !cancelled;n++) {
          const page = await conversationsClient.changes(conversationID, cursor);
          if (cancelled) return;
    ...
    const es = conversationsClient.subscribe(conversationID, {
      onMessage: message => { if (!cancelled) mergeMessages([message]); },
      onFrame: applyFrame,
      onResync: () => { void load(); },
...
export function ConversationChat({
  ...
  const [draft, setDraft] = useState(() => { try {
    if (legacyDrafts) for (const suffix of ["", ":pending"]) {
      const oldKey = `conversations:draft:${conversation.project_id}:${conversation.id}${suffix}`;
      const oldValue = sessionStorage.getItem(oldKey);
      if (oldValue !== null && sessionStorage.getItem(storageKey+suffix) === null) {
        sessionStorage.setItem(storageKey+suffix,oldValue);sessionStorage.removeItem(oldKey);
      }
    }
  ...
  const retryDelivery=async(delivery:MessageDelivery)=>{try{await conversationsClient.retryDelivery(delivery.id);void refreshDeliveries();}catch(err){setSendError(String(err));}};
  ...
  const onAction = async (messageId: number, actionId: string, note: string) => {
    const result = await conversationsClient.act(messageId, actionId, note);
