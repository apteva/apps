...
export function AttachmentContent({attachments=[],chatID}: {attachments?:Attachment[];chatID?:string}) {
 ...
 return <>{attachments.map((item,index)=><div className="chat-message-attachment" key={item.id||index}>
 {item.type==="image"&&/^data:image\/(png|jpeg|gif|webp);base64,/i.test(item.data_url??"")
 ? <button type="button" className="chat-image-preview" aria-label={t("composer.enlarge",{name:item.name||t("attachment.image")})} onClick={()=>setZoom(item)}><img src={item.data_url} alt={item.name||t("attachment.image")} loading="lazy"/></button>
 : <span className="chat-file-icon" aria-hidden="true">▤</span>}
 <div className="chat-file-info"><span>{item.name||t("attachment.image")}</span>{item.size!=null&&<small>{item.mime_type} · {(item.size/1024).toFixed(1)} KB</small>}</div>
...
function ImageDialog({item,close,download,label,downloadLabel}:{item:Attachment;close:()=>void;download:()=>void;label:string;downloadLabel:string}){
 ...
 return <dialog ref={ref} className="chat-image-dialog" onCancel={close} onClick={e=>{if(e.target===e.currentTarget)close()}}><button type="button" aria-label={label} onClick={close}>×</button><img src={item.data_url} alt={item.name||""}/>{item.id&&<button type="button" onClick={download}>{downloadLabel}</button>}</dialog>;
}
