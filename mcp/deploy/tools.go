	...
	return []sdk.Tool{
		{
			...
			Description: "Bind a source to a new deployment. Args: name (slug), source_kind (code|local), source_ref (slug or path), framework? (go|static|blank|''), build_cmd?, start_cmd?, port_hint?, env_json?, domain?, description?",
