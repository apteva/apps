...
const TABS = ["Schema", "Sources", "Resolver modules", "Resolvers", "Authentication", "Function security", "Deploy", "Logs", "Realtime"];
const OPERATIONS = ["find", "search", "get", "count", "aggregate", "aggregate_pipeline", "function", "request", "resolve"];
...
function SourcesTab({ projectId, apiSlug, environment, sources, onRefresh, onError }) {
  ...
  return jsxs("div", { className: "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_380px] gap-4 p-5", children: [jsxs("section", { className: panelClass, children: [jsx("h2", { className: "font-semibold mb-3", children: "Configured sources" }), sources.length === 0 ? jsx(Empty, { children: "No sources configured." }) : jsx("div", { className: "space-y-2", children: sources.map((source) => jsxs("div", { className: "rounded border border-border p-3 flex items-start justify-between gap-3", children: [jsxs("div", { children: [jsx("div", { className: "font-medium", children: source.name }), jsx("div", { className: "text-xs text-text-dim mt-1", children: `${source.kind} · ${JSON.stringify(source.config || {})}` })] }), jsx(StatusPill, { status: source.status })] }, source.id)) })] }), jsxs("form", { className: `${panelClass} space-y-3`, onSubmit: addSource, children: [jsx("h2", { className: "font-semibold", children: "Add or update source" }), jsx(Field, { label: "Name", children: jsx("input", { className: inputClass, value: name, onChange: (event) => setName(event.target.value), placeholder: "orders" }) }), jsx(Field, { label: "Kind", children: jsx("select", { className: inputClass, value: kind, onChange: (event) => { setKind(event.target.value); setConfig(defaults[event.target.value]); }, children: ["database", "tables", "function", "http", "module"].map((value) => jsx("option", { value, children: value }, value)) }) }), jsx(Field, { label: "Configuration", hint: "Module sources pin a published version and map inputs from $parent, $args, or trusted $identity.", children: jsx("textarea", { className: `${inputClass} min-h-[230px] font-mono text-xs`, value: config, onChange: (event) => setConfig(event.target.value), spellCheck: false }) }), jsx(Button, { primary: true, type: "submit", disabled: busy || !name.trim(), children: busy ? "Saving…" : "Save source" })] })] });
}

function ResolverModulesTab({ projectId, apiSlug, environment, modules, sources, resolvers, onRefresh, onError }) {
  const [name, setName] = useState("common.display_name");
  const [description, setDescription] = useState("Reusable display name");
  const [outputType, setOutputType] = useState("String");
  const [inputs, setInputs] = useState('{\n  "first": "String",\n  "last": "String"\n}');
  const [definition, setDefinition] = useState('{\n  "op": "trim",\n  "args": [{\n    "op": "concat",\n    "args": [{ "input": "first" }, { "const": " " }, { "input": "last" }]\n  }]\n}');
  const [testInputs, setTestInputs] = useState('{"first":"Ada","last":"Lovelace"}');
  const [metadata, setMetadata] = useState('{"null_behavior":"propagate","decimal_precision":34,"decimal_scale":12,"rounding_mode":"half_even","timezone":"UTC","completeness":"complete"}');
  const [result, setResult] = useState(null);
  const [busy, setBusy] = useState(false);
  const latest = modules.find((item) => item.name === name);
  const call = async (path, body) => apiFetch(path, projectId, environment, { apiSlug, method: "POST", body: JSON.stringify(body) });
  async function validate() { setBusy(true); onError(null); try { setResult(await call("modules/validate", { inputs: JSON.parse(inputs), definition: JSON.parse(definition) })); } catch (error) { onError(error); } finally { setBusy(false); } }
  async function save(event) { event.preventDefault(); setBusy(true); onError(null); try { setResult(await call("modules", { name, description, inputs: JSON.parse(inputs), output_type: outputType, definition: JSON.parse(definition), deterministic: true, ...JSON.parse(metadata) })); await onRefresh(); } catch (error) { onError(error); } finally { setBusy(false); } }
  async function publish(item) { setBusy(true); onError(null); try { setResult(await call("modules/publish", { name: item.name, version: item.version })); await onRefresh(); } catch (error) { onError(error); } finally { setBusy(false); } }
  async function test() { if (!latest) return; setBusy(true); onError(null); try { const parsed=JSON.parse(testInputs); setResult(await call(Array.isArray(parsed)?"modules/test-batch":"modules/test", Array.isArray(parsed)?{ name:latest.name,version:latest.version,batch:parsed }:{ name:latest.name,version:latest.version,inputs:parsed })); } catch (error) { onError(error); } finally { setBusy(false); } }
  function load(item) { setName(item.name); setDescription(item.description || ""); setOutputType(item.output_type); setInputs(JSON.stringify(item.inputs || {}, null, 2)); setDefinition(JSON.stringify(item.definition || {}, null, 2)); setMetadata(JSON.stringify({ null_behavior:item.null_behavior, decimal_precision:item.decimal_precision, decimal_scale:item.decimal_scale, rounding_mode:item.rounding_mode, timezone:item.timezone, completeness:item.completeness }, null, 2)); setResult(null); }
  function usages(item) { const ids = new Set(sources.filter((source) => source.kind === "module" && source.config?.module === item.name && Number(source.config?.version) === item.version).map((source) => source.id)); return resolvers.filter((resolver) => ids.has(resolver.source_id)).map((resolver) => `${resolver.parent_type}.${resolver.field_name}`); }
  return jsxs("div", { className: "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_430px] gap-4 p-5", children: [
    jsxs("section", { className: panelClass, children: [jsx("h2", { className: "font-semibold", children: "Versioned resolver modules" }), jsx("p", { className: "mt-1 mb-3 text-sm text-text-dim", children: "Reusable typed resolver logic. Clients still query ordinary GraphQL fields; no custom query syntax is introduced." }), modules.length === 0 ? jsx(Empty, { children: "No modules yet." }) : jsx("div", { className: "space-y-2", children: modules.map((item) => jsxs("div", { className: "rounded border border-border p-3", children: [jsxs("div", { className: "flex items-center justify-between gap-2", children: [jsxs("div", { children: [jsx("span", { className: "font-medium", children: item.name }), jsx("span", { className: "ml-2 text-xs text-text-dim", children: `v${item.version} · ${item.output_type}` })] }), jsx(StatusPill, { status: item.status })] }), jsx("p", { className: "mt-1 text-xs text-text-dim", children: item.description || "No description" }), jsx("p", { className: "mt-1 text-xs", children: `Used by: ${usages(item).join(", ") || "no fields"}` }), jsxs("div", { className: "mt-2 flex gap-2", children: [jsx(Button, { onClick: () => load(item), children: "Load" }), item.status !== "published" && jsx(Button, { primary: true, onClick: () => publish(item), disabled: busy, children: "Publish" })] })] }, item.id)) })] }),
    jsxs("form", { className: `${panelClass} space-y-3`, onSubmit: save, children: [jsx("h2", { className: "font-semibold", children: "Module draft" }), jsx(Field, { label: "Module name", children: jsx("input", { className: inputClass, value: name, onChange: (event) => setName(event.target.value), required: true }) }), jsx(Field, { label: "Description", children: jsx("input", { className: inputClass, value: description, onChange: (event) => setDescription(event.target.value) }) }), jsx(Field, { label: "Output type", children: jsx("select", { className: inputClass, value: outputType, onChange: (event) => setOutputType(event.target.value), children: ["String", "ID", "Int", "Float", "Boolean", "JSON", "Decimal", "Date", "DateTime", "Duration"].map((value) => jsx("option", { value, children: value }, value)) }) }), jsx(Field, { label: "Inputs (JSON)", children: jsx("textarea", { className: `${inputClass} min-h-[100px] font-mono text-xs`, value: inputs, onChange: (event) => setInputs(event.target.value), spellCheck: false }) }), jsx(Field, { label: "Runtime metadata (JSON)", hint:"Null behavior, exact Decimal precision/scale/rounding, IANA timezone, and completeness are versioned with the module.", children:jsx("textarea",{className:`${inputClass} min-h-[120px] font-mono text-xs`,value:metadata,onChange:(event)=>setMetadata(event.target.value),spellCheck:false}) }), jsx(Field, { label: "Typed expression (JSON)", hint: "Supports calls, arithmetic, comparisons, boolean logic, dates/durations, if, coalesce, concat, round, min/max/sum, length and string normalization. No JavaScript or eval.", children: jsx("textarea", { className: `${inputClass} min-h-[240px] font-mono text-xs`, value: definition, onChange: (event) => setDefinition(event.target.value), spellCheck: false }) }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { type: "button", onClick: validate, disabled: busy, children: "Validate" }), jsx(Button, { primary: true, type: "submit", disabled: busy || !name, children: "Save new draft" })] }), jsx(Field, { label: "Test inputs", hint:"Enter one object or an array of up to 10,000 objects for batch evaluation.", children: jsx("textarea", { className: `${inputClass} min-h-[75px] font-mono text-xs`, value: testInputs, onChange: (event) => setTestInputs(event.target.value), spellCheck: false }) }), jsx(Button, { type: "button", onClick: test, disabled: busy || !latest, children: "Test latest version" }), result && jsx("pre", { className: "rounded bg-surface-2 p-3 text-xs overflow-auto", children: JSON.stringify(result, null, 2) })] })
  ] });
}
...
function ResolversTab({ projectId, apiSlug, environment, sources, resolvers, onRefresh, onError }) {
  ...
  return jsxs("div", { className: "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_430px] gap-4 p-5", children: [jsxs("section", { className: panelClass, children: [jsx("h2", { className: "font-semibold mb-3", children: "Resolver bindings" }), resolvers.length === 0 ? jsx(Empty, { children: "No resolvers configured." }) : jsx("div", { className: "overflow-x-auto", children: jsxs("table", { className: "w-full text-sm", children: [jsx("thead", { className: "text-left text-text-dim", children: jsx("tr", { children: ["Field", "Source", "Operation", "Config"].map((label) => jsx("th", { className: "pb-2 pr-3 font-normal", children: label }, label)) }) }), jsx("tbody", { children: resolvers.map((item) => jsx("tr", { className: "border-t border-border", children: [jsx("td", { className: "py-2 pr-3 font-mono", children: `${item.parent_type}.${item.field_name}` }), jsx("td", { className: "py-2 pr-3", children: item.source || `#${item.source_id}` }), jsx("td", { className: "py-2 pr-3", children: item.operation }), jsx("td", { className: "py-2 text-xs text-text-dim max-w-[260px] truncate", children: JSON.stringify(item.config || {}) })] }, item.id)) })] }) })] }), jsxs("form", { className: `${panelClass} space-y-3`, onSubmit: saveResolver, children: [jsx("h2", { className: "font-semibold", children: "Bind a GraphQL field" }), jsxs("div", { className: "grid grid-cols-2 gap-3", children: [jsx(Field, { label: "Parent type", children: jsx("input", { className: inputClass, value: parentType, onChange: (event) => setParentType(event.target.value), placeholder: "Query" }) }), jsx(Field, { label: "Field name", children: jsx("input", { className: inputClass, value: fieldName, onChange: (event) => setFieldName(event.target.value), placeholder: "orders" }) })] }), jsx(Field, { label: "Source", children: jsx("select", { className: inputClass, value: source, onChange: (event) => setSource(event.target.value), children: [jsx("option", { value: "", children: "Select source" }), sources.map((item) => jsx("option", { value: item.name, children: `${item.name} (${item.kind})` }, item.id))] }) }), jsx(Field, { label: "Operation", children: jsx("select", { className: inputClass, value: operation, onChange: (event) => setOperation(event.target.value), children: OPERATIONS.map((item) => jsx("option", { value: item, children: item }, item)) }) }), jsx(Field, { label: "Operation config", hint: 'Tables aggregate handles one table. aggregate_pipeline runs one release-owned, parameterized multi-table query while clients use ordinary typed GraphQL fields. Relations support relation_filter with and/or/not, parent/coalesce, and exists/not_exists.', children: jsx("textarea", { className: `${inputClass} min-h-[190px] font-mono text-xs`, value: config, onChange: (event) => setConfig(event.target.value), spellCheck: false }) }), jsx(Button, { primary: true, type: "submit", disabled: busy || !fieldName.trim() || !source, children: busy ? "Saving…" : "Save resolver" })] })] });
}
...
function AuthenticationTab({ projectId, apiSlug, onError }) {
  ...
  return jsxs("form", { onSubmit: save, className: "p-5 space-y-4 max-w-3xl", children: [
    ...
    mode === "auth" && jsxs("div", { className: `${panelClass} space-y-4`, children: [
      ...
      jsx("p", { className: "text-sm text-text-dim", children: `Send the user's Auth bearer token to this endpoint. Authenticated subscriptions use ${API}/public/realtime/${apiSlug} with graphql-transport-ws and the token in connection_init.` }),
    ] }),
...
function DeployTab({ projectId, apiSlug, environment, schema, schemas, releases, onRefresh, onError }) {
  const [busy, setBusy] = useState(false);
  const [limits, setLimits] = useState(JSON.stringify({ max_cost:100000, max_depth:12, max_rows:10000, max_nested_resolvers:2000, max_response_bytes:4194304, max_execution_ms:15000, max_parallelism:8, default_list_size:100 }, null, 2));
  async function publish(version) { setBusy(true); onError(null); try { await apiFetch("schema/publish", projectId, environment, { apiSlug, method: "POST", body: JSON.stringify({ version, limits: JSON.parse(limits) }) }); await onRefresh(); } catch (error) { onError(error); } finally { setBusy(false); } }
  async function rollback(version) { setBusy(true); onError(null); try { await apiFetch("release/rollback", projectId, environment, { apiSlug, method: "POST", body: JSON.stringify({ version }) }); await onRefresh(); } catch (error) { onError(error); } finally { setBusy(false); } }
  return jsxs("div", { className: "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_420px] gap-4 p-5", children: [jsxs("div", { className:"space-y-4", children:[jsxs("section", { className: panelClass, children: [jsx("h2", { className: "font-semibold", children: "Atomic environment release" }), jsx("p", { className:"mt-1 text-sm text-text-dim", children:"Publishing pins the schema, all sources and resolver bindings, security policy, limits, and published calculation modules as one immutable release." }), jsxs("div", { className: "mt-3 flex flex-wrap items-center gap-3", children: [jsx(StatusPill, { status: releases[0]?.status || "not deployed" }), releases[0] && jsx("span", { className:"text-sm", children:`Release v${releases[0].version} · schema v${releases[0].schema_version}` }), schema && jsx(Button, { primary: true, onClick: () => publish(schema.version), disabled: busy || schema.status === "invalid", children: busy ? "Publishing…" : "Publish current draft" })] })] }), jsxs("section", { className: panelClass, children: [jsx("h2", { className: "font-semibold mb-3", children: "Release history" }), releases.length === 0 ? jsx(Empty, { children: "No atomic releases yet." }) : releases.map((item) => jsxs("div", { className:"flex items-center justify-between gap-3 border-b border-border py-3 last:border-0", children:[jsxs("div", { children:[jsxs("div",{className:"flex gap-2",children:[jsx("span",{className:"font-medium",children:`Release v${item.version}`}),jsx(StatusPill,{status:item.status})]}),jsx("div",{className:"text-xs text-text-dim mt-1",children:`schema v${item.schema_version} · ${item.source_count} sources · ${item.resolver_count} resolvers · ${item.module_count} modules · ${item.checksum?.slice(0,12)}`})]}), item.status !== "published" && jsx(Button,{onClick:()=>rollback(item.version),disabled:busy,children:"Rollback"})] },item.id))] })] }), jsxs("aside",{className:`${panelClass} space-y-3`,children:[jsx("h2",{className:"font-semibold",children:"Execution limits"}),jsx("p",{className:"text-sm text-text-dim",children:"Cardinality-aware cost, row, resolver, response-size, deadline, and bounded parallelism limits are pinned into the release."}),jsx("textarea",{className:`${inputClass} min-h-[320px] font-mono text-xs`,value:limits,onChange:(event)=>setLimits(event.target.value),spellCheck:false}),jsx("p",{className:"text-xs text-text-dim",children:"Changing these values has no effect on live traffic until you publish a release."})]})] });
}

function LogsTab({ logs, onRefresh, busy }) {
  return jsx("div", { className: "p-5", children: jsxs("section", { className: panelClass, children: [jsxs("div", { className: "flex items-center justify-between mb-3", children: [jsx("h2", { className: "font-semibold", children: "Production execution telemetry" }), jsx(Button, { onClick: onRefresh, disabled: busy, children: "Refresh" })] }), logs.length === 0 ? jsx(Empty, { children: "No GraphQL requests recorded yet." }) : jsx("div", { className: "overflow-x-auto", children: jsxs("table", { className: "w-full text-sm", children: [jsx("thead", { className: "text-left text-text-dim", children: jsx("tr", { children: ["Time", "Operation", "Release", "Status", "Duration", "Rows", "Resolvers", "Bytes", "Sources", "Request"].map((label) => jsx("th", { className: "pb-2 pr-3 font-normal", children: label }, label)) }) }), jsx("tbody", { children: logs.map((item) => jsx("tr", { className: "border-t border-border", children: [jsx("td", { className: "py-2 pr-3 text-xs", children: item.created_at ? new Date(item.created_at).toLocaleString() : "" }), jsx("td", { className: "py-2 pr-3", title:item.operation_hash, children: item.operation_name || item.operation_hash?.slice(0,10) || "-" }), jsx("td", { className:"py-2 pr-3",children:item.api_release ? `v${item.api_release}`:"legacy" }), jsx("td", { className: `py-2 pr-3 ${item.status_code >= 400 ? "text-red-300" : "text-green-300"}`, title:(item.error_codes||[]).join(", "), children: item.status_code }), jsx("td", { className: "py-2 pr-3", children: `${item.duration_ms} ms` }), jsx("td",{className:"py-2 pr-3",children:item.row_count}),jsx("td",{className:"py-2 pr-3",children:item.resolver_count}),jsx("td",{className:"py-2 pr-3",children:item.response_bytes}),jsx("td",{className:"py-2 pr-3 text-xs",children:Object.entries(item.source_timings||{}).map(([name,value])=>`${name} ${Number(value).toFixed(1)}ms`).join(" · ")||"-"}),jsx("td", { className: "py-2 text-xs max-w-[180px] truncate", title:`${item.authorization_scope || ""} ${item.error || ""}`, children: item.request_id || "-" })] }, item.id)) })] }) })] }) });
}
...
export default function GraphQLPanel({ projectId }) {
  const [environment, setEnvironment] = useState("development"); const [apiSlug, setApiSlug] = useState("default"); const [apis, setApis] = useState([]); const [tab, setTab] = useState("Schema"); const [schema, setSchema] = useState(null); const [schemas, setSchemas] = useState([]); const [releases, setReleases] = useState([]); const [sdl, setSdl] = useState(""); const [sources, setSources] = useState([]); const [resolvers, setResolvers] = useState([]); const [modules, setModules] = useState([]); const [logs, setLogs] = useState([]); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  ...
  const load = async () => {
    ...
    const [active, versions, sourceRows, resolverRows, moduleRows, logRows, releaseRows] = results.map((item) => item.status === "fulfilled" ? item.value : {});
    setSchema(active.schema || versions.schemas?.[0] || null); setSdl(active.schema?.sdl || versions.schemas?.[0]?.sdl || ""); setSchemas(versions.schemas || []); setSources(sourceRows.sources || []); setResolvers(resolverRows.resolvers || []); setModules(moduleRows.modules || []); setLogs(logRows.logs || []); setReleases(releaseRows.releases || []); setLoading(false);
  };
  ...
  const activeTab = useMemo(() => {
    ...
    if (tab === "Resolver modules") return jsx(ResolverModulesTab, { ...common, apiSlug, modules, sources, resolvers });
    ...
    if (tab === "Deploy") return jsx(DeployTab, { ...common, apiSlug, schema, schemas, releases });
  ...
  }, [tab, projectId, apiSlug, environment, schema, schemas, releases, sdl, sources, resolvers, modules, logs, loading]);
