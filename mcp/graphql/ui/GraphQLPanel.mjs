...
function ResolverModulesTab({ projectId, apiSlug, environment, modules, sources, resolvers, onRefresh, onError }) {
  ...
  async function save(event) { event.preventDefault(); setBusy(true); onError(null); try { setResult(await call("modules", { name, description, inputs: JSON.parse(inputs), output_type: outputType, definition: JSON.parse(definition), deterministic: true })); await onRefresh(); } catch (error) { onError(error); } finally { setBusy(false); } }
  ...
  async function test() { if (!latest) return; setBusy(true); onError(null); try { setResult(await call("modules/test", { name: latest.name, version: latest.version, inputs: JSON.parse(testInputs) })); } catch (error) { onError(error); } finally { setBusy(false); } }
  function load(item) { setName(item.name); setDescription(item.description || ""); setOutputType(item.output_type); setInputs(JSON.stringify(item.inputs || {}, null, 2)); setDefinition(JSON.stringify(item.definition || {}, null, 2)); setResult(null); }
  ...
  return jsxs("div", { className: "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_430px] gap-4 p-5", children: [
    ...
    jsxs("form", { className: `${panelClass} space-y-3`, onSubmit: save, children: [jsx("h2", { className: "font-semibold", children: "Module draft" }), jsx(Field, { label: "Module name", children: jsx("input", { className: inputClass, value: name, onChange: (event) => setName(event.target.value), required: true }) }), jsx(Field, { label: "Description", children: jsx("input", { className: inputClass, value: description, onChange: (event) => setDescription(event.target.value) }) }), jsx(Field, { label: "Output type", children: jsx("select", { className: inputClass, value: outputType, onChange: (event) => setOutputType(event.target.value), children: ["String", "ID", "Int", "Float", "Boolean", "JSON", "Decimal"].map((value) => jsx("option", { value, children: value }, value)) }) }), jsx(Field, { label: "Inputs (JSON)", children: jsx("textarea", { className: `${inputClass} min-h-[100px] font-mono text-xs`, value: inputs, onChange: (event) => setInputs(event.target.value), spellCheck: false }) }), jsx(Field, { label: "Typed expression (JSON)", hint: "Supports calls, arithmetic, comparisons, boolean logic, if, coalesce, concat, round, min/max/sum, length and string normalization. No JavaScript or eval.", children: jsx("textarea", { className: `${inputClass} min-h-[240px] font-mono text-xs`, value: definition, onChange: (event) => setDefinition(event.target.value), spellCheck: false }) }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { type: "button", onClick: validate, disabled: busy, children: "Validate" }), jsx(Button, { primary: true, type: "submit", disabled: busy || !name, children: "Save new draft" })] }), jsx(Field, { label: "Test inputs", children: jsx("textarea", { className: `${inputClass} min-h-[75px] font-mono text-xs`, value: testInputs, onChange: (event) => setTestInputs(event.target.value), spellCheck: false }) }), jsx(Button, { type: "button", onClick: test, disabled: busy || !latest, children: "Test latest version" }), result && jsx("pre", { className: "rounded bg-surface-2 p-3 text-xs overflow-auto", children: JSON.stringify(result, null, 2) })] })
  ] });
}
...
export default function GraphQLPanel({ projectId }) {
  const [environment, setEnvironment] = useState("development"); const [apiSlug, setApiSlug] = useState("default"); const [apis, setApis] = useState([]); const [tab, setTab] = useState("Schema"); const [schema, setSchema] = useState(null); const [schemas, setSchemas] = useState([]); const [sdl, setSdl] = useState(""); const [sources, setSources] = useState([]); const [resolvers, setResolvers] = useState([]); const [modules, setModules] = useState([]); const [logs, setLogs] = useState([]); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  ...
  const load = async () => {
    ...
    const [active, versions, sourceRows, resolverRows, moduleRows, logRows] = results.map((item) => item.status === "fulfilled" ? item.value : {});
    setSchema(active.schema || versions.schemas?.[0] || null); setSdl(active.schema?.sdl || versions.schemas?.[0]?.sdl || ""); setSchemas(versions.schemas || []); setSources(sourceRows.sources || []); setResolvers(resolverRows.resolvers || []); setModules(moduleRows.modules || []); setLogs(logRows.logs || []); setLoading(false);
  };
  ...
  }, [tab, projectId, apiSlug, environment, schema, schemas, sdl, sources, resolvers, modules, logs, loading]);
