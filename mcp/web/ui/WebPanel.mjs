      ...
      h("div", { className: "grid gap-2" }, filtered.map((run) => {
        ...
        return h("details", { key: run.id, className: "rounded border border-border bg-bg-card" },
          ...
          h("div", { className: "border-t border-border p-3" },
            h("div", { className: "grid gap-2 text-xs sm:grid-cols-3" }, [["Items", details.item_count], ["Pages", details.page_count], ["Revision", run.extractor_revision], ["Preset", details.preset], ["Backend", details.browser_config?.backend || details.backend], ["Viewport", details.browser_config?.viewport ? `${details.browser_config.viewport.width}×${details.browser_config.viewport.height}` : undefined], ["Current URL", details.current_url]].filter(([, v]) => v !== undefined && v !== "").map(([label, value]) => h("div", { key: label, className: "rounded bg-bg-input p-2" }, h("div", { className: "text-text-dim" }, label), h("div", { className: "mt-0.5 break-all" }, String(value))))),
...
function SchedulesView({ projectId, extractors }) {
  ...
  const [extractorId, setExtractorId] = useState(""); const [name, setName] = useState(""); const [schedule, setSchedule] = useState(JSON.stringify({ kind: "every", every_seconds: 8640 }, null, 2)); const [timezone, setTimezone] = useState("UTC"); const [presetPool, setPresetPool] = useState(""); const [input, setInput] = useState("{}");
