...
export default function TicketsPanel({projectId}:NativePanelProps){
  ...
  return <div className="h-full min-h-0 flex flex-col bg-bg text-text">
    ...
    <main className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[380px_minmax(0,1fr)]">
      ...
      {showAreas?<AreaManager areas={areas} withProject={withProject} reload={loadAreas} close={()=>setShowAreas(false)}/>:creating?<section className="min-h-0 overflow-auto"><TicketEditor draft={draft} setDraft={setDraft} areas={areas} busy={busy} onSave={createTicket} create/></section>:detail&&selected?<section className="min-h-0 flex flex-col">
        <div className="shrink-0 border-b border-border px-4 py-3 flex items-center gap-3"><div><div className="text-xs text-text-muted">{detail.ticket.key}</div><div className="font-semibold">{detail.ticket.title}</div></div>{detail.ticket.requester_crm_contact_id&&<span className="text-[10px] border border-border rounded px-2 py-1">CRM #{detail.ticket.requester_crm_contact_id}</span>}<a href={detail.ticket.portal_url} target="_blank" rel="noreferrer" className="ml-auto text-xs text-accent hover:underline">Open client view</a><button disabled={busy} onClick={saveTicket} className="px-3 py-1.5 text-xs bg-accent text-bg rounded font-medium disabled:opacity-50">{busy?"Saving…":"Save"}</button></div>
        <div className="flex-1 min-h-0 overflow-auto"><TicketEditor draft={draft} setDraft={setDraft} areas={areas} busy={busy} onSave={saveTicket}/><div className="border-t border-border p-4">
          ...
          <textarea value={composer} onChange={e=>setComposer(e.target.value)} placeholder={internal?"Visible only to the team and agents":"Visible to the client"} className="w-full min-h-24 bg-bg-input border border-border rounded p-3 text-sm resize-y"/>
          <div className="mt-2 flex items-center gap-2"><label className="px-3 py-1.5 text-xs border border-border rounded cursor-pointer hover:bg-bg-input">Attach file<input type="file" multiple className="hidden" onChange={e=>void upload(e.target.files)}/></label><button disabled={busy||!composer.trim()} onClick={addComment} className="px-3 py-1.5 text-xs bg-accent text-bg rounded font-medium disabled:opacity-50">{internal?"Add note":"Send reply"}</button></div>
