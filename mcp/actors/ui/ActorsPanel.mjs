...
function ActorsView({ projectId, refreshRuns }) {
  ...
  const load = useCallback(async () => {
    setError("");
    try { const body = await request("/actors", projectId); setActors(body.actors || []); }
  ...
  async function save(event) {
    ...
    try {
      ...
      const result = await request("/actors", projectId, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function run(id) {
    ...
    try {
      const result = await request("/actors/run", projectId, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actor_id: id, operation, preset: preset || undefined, input: parseJSON(input, "Run input") }) });
  ...
  async function remove(record) {
    ...
    setBusy(true); setError(""); setMessage("");
    try { await request(`/actors/${record.id}`, projectId, { method: "DELETE" }); setMessage(`Deleted ${record.name}.`); if (editing?.id === record.id) edit(null); await load(); }
    catch (err) { setError(String(err.message || err)); } finally { setBusy(false); }
  }

  return h("div", { className: "grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,1fr)]" },
    h("section", null,
      h("div", { className: "mb-3 flex items-center" }, h("div", null, h("h2", { className: "font-semibold" }, "Actors"), h("p", { className: "text-xs text-text-muted" }, "Reusable browser workflows with immutable run snapshots.")), h("button", { type: "button", onClick: () => edit(null), className: "ml-auto rounded bg-accent px-3 py-1.5 text-sm text-white" }, "New actor")),
      loading ? h("div", { className: "py-10 text-center text-sm text-text-muted" }, "Loading actors…") : actors.length === 0 ? h(Empty, { title: "No actors yet", detail: "Create one from the version 1 starter definition." }) :
    ...
    h("section", { className: "rounded border border-border bg-bg-card p-4" },
      h("h2", { className: "font-semibold" }, editing ? `Edit ${editing.name}` : "New actor"),
    ...
    h(Notice, { error }), loading ? h("div", { className: "py-12 text-center text-sm text-text-muted" }, "Loading runs…") : filtered.length === 0 ? h(Empty, { title: "No matching runs", detail: "Actor and Actors tool activity appears here." }) :
      h("div", { className: "grid gap-2" }, filtered.map((run) => {
        ...
        return h("details", { key: run.id, className: "rounded border border-border bg-bg-card" },
          h("summary", { className: "cursor-pointer list-none p-3" }, h("div", { className: "flex gap-2" }, h("div", { className: "min-w-0 flex-1" }, h("div", { className: "text-sm font-medium capitalize" }, run.kind === "actor" ? `Actor #${run.actor_id}` : run.kind), h("div", { className: "mt-1 text-xs text-text-muted break-words" }, run.summary || `Run #${run.id}`)), h("span", { className: `h-fit rounded border px-2 py-0.5 text-xs ${statusClass(run.status)}` }, run.status)), h("div", { className: "mt-2 text-xs text-text-dim" }, `${relativeTime(run.created_at)}${run.duration_ms != null ? ` • ${duration(run.duration_ms)}` : ""}`)),
          h("div", { className: "border-t border-border p-3" },
            ...
            live && h("div", { className: "mt-3" }, h("div", { className: "mb-1 text-xs text-text-muted" }, "Live Computer view"), h("img", { src: computerScreenshotURL(details.session_id, projectId), alt: "Live actor browser", className: "max-h-80 rounded border border-border object-contain" })),
            ...
            h("div", { className: "mt-3 flex flex-wrap gap-2" }, [["Read dataset", endpoint(`/runs/${run.id}/dataset`, projectId)], ["Download JSONL", dataset], ["Download CSV", csv], ["Open trace", trace]].filter(([, href]) => href).map(([label, href]) => h("a", { key: label, href, target: "_blank", rel: "noopener noreferrer", className: "rounded border border-border px-2 py-1 text-xs text-accent" }, label)), (run.status === "queued" || run.status === "running") && h("button", { disabled: busy === run.id, onClick: () => action(run.id, "cancel"), className: "rounded border border-red px-2 py-1 text-xs text-red" }, "Cancel"), ["completed", "failed", "cancelled"].includes(run.status) && run.actor_id && h("button", { disabled: busy === run.id, onClick: () => action(run.id, "retry"), className: "rounded border border-border px-2 py-1 text-xs" }, "Retry"))
          )
        );
      })),
...
function SchedulesView({ projectId, actors }) {
...
function TasksView({ projectId, actors, onRun }) {
  ...
  const [busy, setBusy] = useState(false);
  const load = useCallback(async () => {
    try { const data = await request("/tasks", projectId); setTasks(data.tasks || []); }
  ...
  async function save(event) {
    ...
    try {
      await request("/tasks", projectId, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, actor_id: Number(actorId), operation, input: parseJSON(input, "Input") }) });
      setName(""); await load();
  ...
  async function action(task, kind) {
    ...
    try {
      if (kind === "run") {
        await request(`/tasks/${task.id}/run`, projectId, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }); onRun();
      } else {
        await request(`/tasks/${task.id}`, projectId, { method: "DELETE" }); await load();
      }
    } catch (err) { setError(err.message); } finally { setBusy(false); }
  }
...
function ActorsPanel({ projectId } = {}) {
  const [tab, setTab] = useState("actors"); const [refreshRuns, setRefreshRuns] = useState(0); const [actors, setActors] = useState([]);
  useEffect(() => { request("/actors", projectId).then((body) => setActors(body.actors || [])).catch(() => {}); }, [projectId, tab]);
  return h("div", { className: "h-full min-w-0 overflow-auto bg-bg text-text" },
    h("header", { className: "sticky top-0 z-10 border-b border-border bg-bg px-4 pt-3" }, h("div", null, h("h1", { className: "text-lg font-semibold" }, "Actors"), h("p", { className: "text-xs text-text-muted" }, "Reusable browser workflows, tasks, datasets, and schedules")), h("nav", { className: "mt-3 flex gap-1", "aria-label": "Actors sections" }, [["actors", "Actors"], ["tasks", "Tasks"], ["runs", "Runs"], ["schedules", "Schedules"]].map(([id, label]) => h("button", { key: id, type: "button", onClick: () => setTab(id), className: `border-b-2 px-3 py-2 text-sm ${tab === id ? "border-accent text-text" : "border-transparent text-text-muted hover:text-text"}` }, label)))),
    h("main", { className: "mx-auto w-full max-w-6xl p-4" }, tab === "actors" ? h(ActorsView, { projectId, refreshRuns: () => { setRefreshRuns((v) => v + 1); setTab("runs"); } }) : tab === "tasks" ? h(TasksView, { projectId, actors, onRun: () => { setRefreshRuns((v) => v + 1); setTab("runs"); } }) : tab === "runs" ? h(RunsView, { projectId, refreshToken: refreshRuns }) : h(SchedulesView, { projectId, actors }))
  );
}
...
if (typeof document !== "undefined") {
  ...
  if (element) createRoot(element).render(h(ActorsPanel));
}
