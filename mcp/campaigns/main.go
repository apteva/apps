...
requires:
  ...
  apps:
    - name: messaging
      optional: true
      reason: Receive provider delivery/bounce/complaint/open events for campaign recipient reconciliation; the required Messaging sender binding below is the actual app dependency.
      events:
        - message.event
  integrations:
    - role: crm
