...
func TestHTTPRoutesRegistered(t *testing.T) {
	...
	for _, want := range []string{"/summary", "/series", "/top", "/feed", "/dimensions", "/collect", "/keys", "/keys/revoke", "/capture", "/dashboards", "/dashboards/", "/widgets/", "/query-widget", "/query-dashboard", "/dashboard-filter-options", "/event-specs", "/event-specs/", "/event-spec-violations"} {
		if !got[want] {
