...
type Publication = { id: string; asset_id: string; destination: string; account_ref: string; audience: string; status: string; planned_at: string; actual_at: string; external_post_id: string; external_url: string; evidence_source: string; failure_details: string; legacy_target_id: string };
...
export default function ContentCatalogPanel({ projectId, installId }: { projectId: string; installId: number }) {
  ...
  return <div className="h-full overflow-y-auto bg-bg text-text p-5 space-y-5">
    ...
    {modal === "publication" && selectedAsset && selectedSession && <Modal title={editingPublication ? "Update platform status" : "Add platform to file"} onClose={() => setModal(null)}><PublicationForm key={editingPublication?.id || selectedAsset.id} busy={busy} initial={editingPublication} onSubmit={input => run(async () => { const asset = selectedAsset; await action("content_catalog_asset_publications_record", { asset_id: asset.id, ...input }); setModal(null); await openSession(selectedSession); await openAsset(asset); }, "Platform status saved")} />{error && <p className="mt-2 text-sm text-red-400">{error}</p>}</Modal>}
  </div>;
}
...
function PublicationForm({ busy, initial, onSubmit }: { busy: boolean; initial: Publication | null; onSubmit: (x: Record<string, unknown>) => void }) {
  ...
  return <form className="space-y-3" onSubmit={e => { e.preventDefault(); onSubmit({ ...(initial ? { publication_id: initial.id } : { destination: destination.trim().toLowerCase(), account_ref: account.trim(), audience: audience.trim(), planned_at: planned ? new Date(planned).toISOString() : "" }), status, actual_at: actual ? new Date(actual).toISOString() : "", external_url: url.trim(), external_post_id: postID.trim(), evidence_source: source.trim(), failure_details: failure.trim() }); }}>
    <h3 className="font-semibold">{initial ? `Update ${initial.destination}` : "Record a platform for this file"}</h3>
    <p className="text-xs text-text-muted">This saves a Catalog record only. It does not publish a post.</p>
    ...
    <button className={buttonClass} disabled={busy}>Save platform status</button>
  </form>;
}
