...
func pcbExamples() map[string]any {
	...
	return map[string]any{
		"schema": pcbSchema, "engine": engineVersion, "units": map[string]string{"geometry": "integer nanometres", "rotation": "integer microdegrees", "wiring_canvas": "CSS pixels"}, "example": def, "manufacturable_sensor_node": sensorNodeExample(), "arduino_led_breadboard": arduinoLEDExample(), "wiring_library": wiringLibraryResponse(),
		"operations":    []string{"board.set", "rules.set", "component.add", "component.replace", "component.remove", "placement.set", "net.add", "net.replace", "net.remove", "net.connect", "net.disconnect", "trace.add", "trace.remove", "via.add", "via.remove", "zone.add", "zone.remove", "keepout.add", "keepout.remove", "differential_pair.add", "differential_pair.remove", "net_class.add", "net_class.replace", "net_class.remove", "simulation.set", "wiring.set", "wiring.part.add", "wiring.part.replace", "wiring.part.remove", "wiring.wire.add", "wiring.wire.replace", "wiring.wire.remove"},
		"compatibility": map[string]any{"native": "apteva-pcb/v1 JSON", "v0_5_export": []string{"interactive board SVG", "wiring SVG", "wiring PNG", "tutorial JSON", "tutorial ZIP", "BOM CSV", "Gerber X2", "Excellon", "Gerber job JSON", "fabrication-verification JSON", "release ZIP", "simulation JSON", "firmware-run JSON"}, "adapter_contract": "Import/export adapters are deterministic Apteva-owned parsers and writers; no external ECAD or illustration engine is required."},
	}
}
