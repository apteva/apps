...
test('selection follows visibility changes and paginated lists',async({page})=>{
 ...
 await page.route('**/api/apps/storage/**',async route=>{const r=route.request(),u=new URL(r.url());expect(u.searchParams.get('project_id')).toBe('p1');expect(u.searchParams.get('install_id')).toBe('42');
...
test('a late root refresh cannot overwrite the newly opened folder',async({page})=>{
 let delay=false;
 await page.route('**/api/apps/storage/**',async route=>{const u=new URL(route.request().url()),folder=u.searchParams.get('folder')||u.searchParams.get('parent');if(delay&&folder==='/')await new Promise(r=>setTimeout(r,350));
...
test('batch uploads continue after one failure and folder errors are visible',async({page})=>{
 let posts=0;
 await page.route('**/api/apps/storage/**',async route=>{const r=route.request(),u=new URL(r.url());if(r.method()==='POST'){posts++;return route.fulfill(posts===2?{json:row(2,'good.txt')}:{status:500,body:'write failed'})}return route.fulfill({json:u.pathname.endsWith('/folders')?{folders:[]}:{files:[]}})});
...
test('card events use current file ID and every URL includes scope',async({page})=>{
 let second=0;
 await page.route('**/api/apps/storage/**',async route=>{const u=new URL(route.request().url());expect(u.searchParams.get('project_id')).toBe('p1');expect(u.searchParams.get('install_id')).toBe('42');const id=Number(u.pathname.split('/')[5]);if(id===2)second++;return route.fulfill({json:{file:row(id,id===2?(second>1?'updated.txt':'second.txt'):'first.txt')}})});
 await page.goto('/?card');await expect(page.getByRole('heading',{name:'first.txt'})).toBeVisible();await page.evaluate(()=>{(window as any).selectCard(2)});await expect(page.getByRole('heading',{name:'second.txt'})).toBeVisible();
 await page.evaluate(()=>{(window as any).fireStorageEvent({topic:'file.updated',data:{id:2}})});await expect(page.getByRole('heading',{name:'updated.txt'})).toBeVisible();await expect(page.getByRole('link',{name:'Open'})).toHaveAttribute('href',/project_id=p1&install_id=42/);
});
test('resuming reuses verified parts after a transient failure',async({page})=>{
 ...
 await page.route('**/api/apps/storage/**',async route=>{const req=route.request(),u=new URL(req.url());
 if(u.pathname.endsWith('/folders'))return route.fulfill({json:{folders:[]}});
