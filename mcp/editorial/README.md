...
- Unknown arguments are rejected on both doors, naming what the operation accepts, rather than being ignored — an unrecognised filter used to answer a deliberately narrow query with a full unfiltered page. Over HTTP that is a 400; over MCP the tool call fails. `POST` and `PATCH` read their arguments from the body, so any argument in their query is refused too. The check lives in the shared dispatch because the SDK publishes each tool's `inputSchema` without enforcing it: `additionalProperties: false` tells a client what is accepted, it does not stop the call. There is still no structured filter on an individual custom field (`fields.<key>=<value>`); search reaches their values, but narrowing by one named field is not yet a filter.
