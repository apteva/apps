...
function DecisionMap({ network, points, input, onInput }: { network: Network; points: Point[]; input: number[]; onInput: (p: number[]) => void }) {
  ...
  useEffect(() => {
    ...
    const positive=resolve('--success'), negative=resolve('--recurring'), surface=resolve('--bg-card'), text=resolve('--text');
    probe.remove();
...
function LossChart({ history }: { history: Metric[] }) {
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', display: 'block' }} role="img" aria-label="Actual training and validation binary cross-entropy by epoch">
    ...
    <path d={path('loss')} fill="none" stroke="var(--success)" strokeWidth="2" /><path d={path('validation_loss')} fill="none" stroke="var(--recurring)" strokeWidth="2" strokeDasharray="5 3" />
...
function Neurons({ network, input, spikes, motion }: { network: Network; input: number[]; spikes: boolean; motion: boolean }) {
  ...
  return <div ref={ref}>
    <svg className="graph" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Live neural network. Line color shows weight sign, thickness shows magnitude, and neuron pulses encode actual activation strength.">
      {network.weights.flatMap((layer, l) => layer.flatMap((row, j) => row.slice(0, -1).map((w, i) => {
        ...
        return <line key={`${l}-${j}-${i}`} x1={x1 + r} y1={y1} x2={x2 - r} y2={y2} stroke={w >= 0 ? 'var(--success)' : 'var(--recurring)'} strokeOpacity={fired ? .85 : Math.min(.38, .06 + Math.abs(w) * .08)} strokeWidth={Math.min(3, .5 + Math.abs(w) * .5)} />;
      }))) }
      ...
      {activations.flatMap((layer, l) => layer.map((a, j) => {
        const [x, y] = pos(l, j), fired = spikes && motion && frame.fired[l]?.[j], color = a >= 0 ? 'var(--success)' : 'var(--recurring)', picked = selected[0] === l && selected[1] === j;
    ...
    {tab==='lab'?<div className="layout">
      ...
      <div className="stack">{snapshot&&e&&net?<>
        <section className="box"><div className="section row space"><div><h2>{e.name}</h2><span className="muted">{net.shape.join(' → ')} · {params} parameters · {e.state.config.dataset}</span></div><span className={`badge ${e.status}`}>{e.status}</span></div>
          <div className="pad" style={{paddingBottom:0}}><div className="row space"><div className="legend"><span><i className="dot"/>Positive weight</span><span><i className="dot negative"/>Negative weight</span></div><label className="check"><input type="checkbox" checked={spikes} onChange={ev=>setSpikes(ev.target.checked)}/>Spike view</label></div></div>
