...
func TestPrepareWorkloadRedactsEnvironmentAndStoresOwner(t *testing.T) {
	...
	if len(got.EnvKeys) != 1 || got.EnvKeys[0] != "TOKEN" || got.Env["TOKEN"] != "secret" {
		t.Fatalf("internal environment was not preserved safely: %+v", got)
	}
}
...
func TestExecutionRunsAsIsolatedOwnedContainerAndRetainsLogs(t *testing.T) {
	...
	w.ProjectID = "project-a"
	w.EnvJSON = `{"BASE":"yes"}`
	w.Env = map[string]string{"BASE": "yes"}
	...
	if started.Env["BASE"] != "yes" || started.Env["CI"] != "true" {
		t.Fatalf("execution environment not merged: %+v", started.Env)
	}
