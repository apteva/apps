...
function InvoicesTab({ projectId, apiCall }: { projectId: string; apiCall: ApiCall }) {
  ...
  const sendPaymentLink = () => {
    if (detail) setShowPaymentLink(true);
  };
        ...
        {(invoice.status === "open" || invoice.status === "uncollectible") && (
          <>
            ...
            <button
              ...
              className="px-3 py-1 text-sm border border-border rounded hover:bg-bg-input"
              title="Generate a Stripe-hosted payment URL (requires payment_processor integration)"
            >
              Send payment link
            </button>
          </>
...
function SendPaymentLinkModal({
  ...
  const generate = async () => {
    ...
    try {
      ...
      const r = await fetch(
        `/api/apps/billing/mcp?project_id=${encodeURIComponent(projectId)}`,
        {
          method: "POST",
          credentials: "same-origin",
      ...
      if (!inner) throw new Error("No response body from billing");
      const payload = JSON.parse(inner) as { url?: string; error?: string };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4 space-y-3 text-sm">
          {phase === "loading" && (
            <div className="text-text-muted">Generating Stripe Checkout Session…</div>
          )}
          {phase === "error" && (
            <>
              ...
              {/no payment_processor bound/.test(error) && (
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-bg-input border border-border rounded px-2 py-1 text-xs font-mono"
                />
                <button
                  type="button"
                  onClick={copy}
...
function PaymentMethodSetupModal({
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const res = await apiCall<{ customers: Customer[] }>(
          ...
          "/customers",
          undefined,
          query.trim() ? { q: query.trim() } : { limit: "20" },
        );
        if (!cancelled) setCustomers((res.customers || []).slice(0, 20));
      } catch {
  ...
  const paymentTypes = () => {
    const types: string[] = [];
    if (card) types.push("card");
    if (sepaDebit) types.push("sepa_debit");
    return types;
  };
              ...
              <>
                ...
                <div className="border border-border rounded max-h-48 overflow-auto">
                  ...
                  {!loadingCustomers && customers.length === 0 && (
                    <div className="p-3 text-xs text-text-muted">No customers.</div>
                  )}
                  {!loadingCustomers &&
                    customers.map((c) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 border-b border-border last:border-b-0 hover:bg-bg-input"
                      >
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <label className="flex items-center gap-2 text-sm text-text">
                <input
                  type="checkbox"
                  checked={sepaDebit}
                  onChange={(e) => setSepaDebit(e.target.checked)}
                />
                SEPA debit
              </label>
              <label className="flex items-center gap-2 text-sm text-text">
