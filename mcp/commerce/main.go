...
func (a *App) MCPTools() []sdk.Tool {
	return []sdk.Tool{
		...
		{Name: "commerce_marketing_channel_options", Description: "List ad accounts and tracking sources available through the optional Ads app, across every supported platform (Meta Pixels, Google conversion actions). Args: store_id.", InputSchema: schemaObject(map[string]any{"store_id": typ("integer")}, []string{"store_id"}), Handler: a.toolMarketingChannelOptions},
		{Name: "commerce_marketing_channel_get", Description: "Return configured marketing channels for a store. Args: store_id, provider? (meta|google). Returns channels[] plus a single channel for the requested provider.", InputSchema: schemaObject(map[string]any{"store_id": typ("integer")}, []string{"store_id"}), Handler: a.toolMarketingChannelGet},
		{Name: "commerce_marketing_channel_configure", Description: "Select or create a tracking source (Meta Pixel or Google conversion action) and install consent-gated browser tracking on the Content storefront. The provider is derived from the ad account.", InputSchema: schemaObject(marketingChannelProps(), []string{"store_id", "ad_account_id"}), Handler: a.toolMarketingChannelConfigure},
		{Name: "commerce_marketing_channel_disconnect", Description: "Disable a store marketing channel and remove it from the published storefront. Args: store_id, provider? (meta|google, defaults to meta).", InputSchema: schemaObject(map[string]any{"store_id": typ("integer")}, []string{"store_id"}), Handler: a.toolMarketingChannelDisconnect},
		{Name: "commerce_marketing_channel_public_get", Description: "Return browser-public tracking configuration for a store as channels[], with the Meta channel mirrored at the top level for storefronts published before 0.9.0. Args: store_id.", InputSchema: schemaObject(map[string]any{"store_id": typ("integer")}, []string{"store_id"}), Handler: a.toolMarketingChannelPublicGet},
	}
}
