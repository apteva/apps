...
func (a *App) instructionTools() []sdk.Tool {
	return []sdk.Tool{
		...
		},
		{
			...
			Description: "Fork a new version. The previous version stays queryable; templates that pinned it keep working. body.response may independently configure note and files expectations; legacy body.response_mode remains readable. Args: id, body, default_result_key?. Returns {instruction, new_version}.",
			InputSchema: schemaObject(map[string]any{
				"id":                 map[string]any{"type": "integer"},
				"body":               map[string]any{"type": "object"},
				"default_result_key": map[string]any{"type": "string"},
