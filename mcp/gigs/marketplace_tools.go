...
func (a *App) marketplaceTools() []sdk.Tool {
	...
	return []sdk.Tool{
		...
		{Name: "gigs_create_from_offer", Description: "Create and optionally assign a gig from a published standard offer package. Supports scheduled_for, soft due_at, optional hard access_expires_at or access_grace_days, and deprecated deadline_at as a due alias. Returns {gig,assignment?,recommendation}.", InputSchema: obj(map[string]any{"offer_id": integer, "offer_slug": stringP, "package_id": integer, "package_slug": stringP, "worker_id": integer, "quantity": number, "vars": object, "notify_worker": boolean, "public_domain_id": integer, "scheduled_for": stringP, "due_at": stringP, "deadline_at": stringP, "access_expires_at": stringP, "access_grace_days": integer, "priority": stringP}), Handler: a.toolGigsCreateFromOffer},
		...
		{Name: "contracts_dispatch_milestone", Description: "Dispatch a pending contract milestone as a worker gig with snapshotted compensation. Supports scheduled_for, soft due_at, optional hard access_expires_at or access_grace_days, and deprecated deadline_at as a due alias.", InputSchema: obj(map[string]any{"contract_id": integer, "milestone_id": integer, "vars": object, "notify_worker": boolean, "public_domain_id": integer, "scheduled_for": stringP, "due_at": stringP, "deadline_at": stringP, "access_expires_at": stringP, "access_grace_days": integer, "priority": stringP}, "contract_id", "milestone_id"), Handler: a.toolContractsDispatchMilestone},
...
func (a *App) toolGigsCreateFromOffer(ctx *sdk.AppCtx, args map[string]any) (any, error) {
	...
	createArgs := map[string]any{"_project_id": pid, "template_id": o.TemplateID, "vars": mapArg(args, "vars"), "worker_id": int64Arg(args, "worker_id"), "notify_worker": boolArg(args, "notify_worker", false), "public_domain_id": int64Arg(args, "public_domain_id"), "scheduled_for": strArg(args, "scheduled_for"), "due_at": strArg(args, "due_at"), "deadline_at": strArg(args, "deadline_at"), "access_expires_at": strArg(args, "access_expires_at"), "access_grace_days": intArg(args, "access_grace_days", 0), "priority": strArg(args, "priority"), "_compensation_quote": quote, "_offer_id": o.ID, "_offer_package_id": p.ID}
